    ...
    for node in ast.walk(tree):
      if isinstance(node, ast.FunctionDef):
          if node.name == "upgrade":
              upgrade_is_empty = not any(not isinstance(stmt, ast.Pass) for stmt in node.body)
          elif node.name == "downgrade":
              downgrade_is_empty = not any(not isinstance(stmt, ast.Pass) for stmt in node.body)

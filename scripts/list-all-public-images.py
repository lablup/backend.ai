...
async def get_all_image_summaries(sess):
    result = await fetch(DOCKER_HUB_URL, sess)
    return [(f'lablup/{image["name"]}', image["last_updated"]) for image in result["results"]]


async def get_auth_token(image_name, sess):
    result = await fetch(AUTH_TOKEN_URL.format(image_name), sess)
    return result["token"]
...
async def get_all_tags(image_name, sess, token):
    try:
        result = await fetch(
            TAG_LIST_URL.format(image_name), sess, headers={"Authorization": f"Bearer {token}"}
        )
...
async def get_image_detail(image_name, sess):
    ...
    digests = await asyncio.gather(*[get_digest(image_name, tag, sess, token) for tag in tags])
    return ImageDetail(image_name, [(tag, digest) for tag, digest in zip(tags, digests)])


async def list_all_public_images():
    scheduler = await aiojobs.create_scheduler()
    image_filter = lambda name: (name.startswith("lablup/kernel-") and "base" not in name)
    async with aiohttp.ClientSession() as sess:
        ...
        image_details = await asyncio.gather(*[job.wait() for job in jobs])
        table = []

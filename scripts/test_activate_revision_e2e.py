#!/usr/bin/env python3
"""
ActivateRevision appproxy registration E2E test (BA-5557).

Verifies that deployments entering DEPLOYING via ActivateRevision
get their appproxy endpoints registered and reach READY without DEGRADED.

Flow: create_deployment → wait READY → add_revision v2 → activate v2 → wait READY

Uses a simple mock HTTP server (python3 -m http.server) instead of vLLM.
"""
import json
import sys
import time
...
DOMAIN_NAME = "default"
IMAGE_ID = "04f5334c-9293-4263-a89c-86e2c1bae99e"  # python:3.13-ubuntu24.04
MODEL_VFOLDER_ID = "0a065d0cc25b4463907854f43463b32a"  # mock http server (vllm-test-model vfolder)
...
def make_revision_input(name):
    return {
        ...
        "model_mount_config": {
            ...
            "definition_path": "mock-model-definition.yaml",
        },
    }
...
def step_create_deployment():
    ...
    body = {
        "metadata": {
            ...
            "name": f"ba5557-e2e-{int(time.time())}",
            "tags": ["e2e-test", "ba-5557"],
        },
        "network_access": {
        ...
        "initial_revision": make_revision_input("v1"),
    }
...
def step_poll_deployment_status(deployment_id, target_status, step_label):
    ...
    while time.time() - start_time < POLL_TIMEOUT_SECONDS:
        ...
        print(
            ...
            f"revision={revision_name}, url={url}"
        )
        if status == target_status:
            print(f"  -> Reached target status: {target_status}")
            return result
        if status in ("DESTROYED", "ERROR", "DEGRADED"):
...
def step_add_revision(deployment_id):
    ...
    body = {
        "revision": make_revision_input("v2"),
    }
    response = requests.post(
        str(API_ENDPOINT / rel_url[1:]),
        headers=make_headers("POST", rel_url),
...
def main():
    print("BA-5557 ActivateRevision Appproxy Registration E2E Test")
    print("=" * 60)
    ...
    # Step 4: Activate revision v2 (triggers DEPLOYING via ActivateRevision)
    step_activate_revision(deployment_id, revision_id)

    # Step 5: Poll until rolling update completes (back to READY)
    # This is the critical step — before BA-5557 fix, deployment would get
    # stuck in DEGRADED because appproxy endpoint was not registered.
    ...
    if current_revision and current_revision["id"] == revision_id:
        print("\n" + "=" * 60)
        print("[PASS] ActivateRevision completed. Current revision is v2.")
        print("  Deployment did NOT get stuck in DEGRADED.")
    else:
        print("\n" + "=" * 60)

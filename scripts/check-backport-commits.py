...
backports_since = subprocess.check_output([
    "git",
    "log",
    f"{prev_release}..{major}.{minor}",
    "--oneline",
]).decode("utf-8")
release_date = (
...
merges_since_json = subprocess.check_output([
    "gh",
    "pr",
    "list",
    "--search",
    f"merged:>={release_date} milestone:{major}.{minor}",
    "-s",
    "merged",
    "--json",
    "number,mergeCommit",
...
for pr_info in merges_since:
    ...
    if f"#{pr_number}" not in backports_since:
        print(
            f"Could not find PR #{pr_number} (https://github.com/lablup/backend.ai/pull/{pr_number}, main commit SHA {commit_sha}) from backport list"
        )
        failed_commits += 1
    else:
...
if failed_commits > 0:
    print(
        f"Among the {len(merges_since)} merged PRs we found {failed_commits} are not yet backported"
    )
else:

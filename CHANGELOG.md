...
* Add Domain (tenant) CRUD client to Client SDK v2 with create, get, search, update, delete, and purge operations. ([#9059](https://github.com/lablup/backend.ai/issues/9059))
...
* Add Prometheus query preset DB schema and ORM Row model (BEP-1050). This lays the foundation for the preset-based Prometheus query system, enabling administrators to manage reusable metric query templates. ([#9364](https://github.com/lablup/backend.ai/issues/9364))
...
* Add anonymous client to v2 client SDK for unauthenticated endpoints ([#9478](https://github.com/lablup/backend.ai/issues/9478))
...
* Add `permissions` and `users` connection fields to `RoleGQL` with Relay-style pagination support.
  ...
  - `Role.users`: Returns `RoleAssignmentConnection` with assignment metadata (granted_by, granted_at) and user information
    ...
    - Access user details via nested `user` field on each assignment node ([#9518](https://github.com/lablup/backend.ai/issues/9518))
...
* Replace `DeploymentHandler`'s `next_status` and `failure_status` with `status_transitions` ([#9441](https://github.com/lablup/backend.ai/issues/9441))
...
* Migrate service (model serving) handlers from old-style module-level functions to the new ApiHandler class pattern with constructor DI, Pydantic DTOs, and APIResponse. ([#9447](https://github.com/lablup/backend.ai/issues/9447))
...
* Introduce `DeploymentLifecycleStatus` and `DeploymentSubStatus` types to deployment handler interface, replacing raw `EndpointLifecycle` returns with structured transition targets that can carry sub-status context ([#9361](https://github.com/lablup/backend.ai/issues/9361))
...
* Fix `session`, `dotfile` endpoints return 400 Malformed body ([#9588](https://github.com/lablup/backend.ai/issues/9588))

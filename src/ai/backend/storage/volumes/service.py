...
class VolumeService:
    ...
    async def _get_capabilities(self, volume_id: str) -> list[str]:
    ...
    async def _delete_vfolder(
        ...
        task_map[vfolder_id] = current_task

        try:
            async with self._volume_pool.get_volume(str(volume_id)) as volume:
    ...
    async def get_volume(self, volume_data: VolumeIdData) -> VolumeMetadata:
        ...
        await log_manager_api_entry(log, "get_volume", volume_data)
        volume = self._volume_pool.get_volume_info(str(volume_id))
        return VolumeMetadata(
            volume_id=volume_id,
            backend=volume.backend,
            path=volume.path,
            fsprefix=volume.fsprefix,
            capabilities=await self._get_capabilities(str(volume_id)),
        )
    ...
    async def delete_vfolder(self, vfolder_data: VFolderIdData) -> None:
        ...
        try:
            ...
            if ongoing_task is not None and ongoing_task.done():
                asyncio.create_task(
                    self._delete_vfolder(task_map=self._deletion_tasks, vfolder_data=vfolder_data)
                )
        return None

...
class HuggingFaceService:
    ...
    async def scan_model(self, registry_name: str, model_id: str) -> ModelInfo:
        ...
        return await self._make_scanner(registry_name).scan_model(model_id)

    async def list_model_files(self, registry_name: str, model_id: str) -> list[FileInfo]:
        ...
        return await self._make_scanner(registry_name).list_model_files_info(model_id)

    async def get_download_url(self, registry_name: str, model_id: str, filename: str) -> str:
        ...
        return self._make_scanner(registry_name).get_download_url(model_id, filename)

    async def import_model(
        self, registry_name: str, model_id: str, storage_name: str, bucket_name: str
    ) -> uuid.UUID:
        ...
        async def _import_model(reporter: ProgressReporter) -> None:
            try:
                ...
                file_infos = await scanner.list_model_files_info(model_id)
    ...
    async def import_models_batch(
        self, registry_name: str, model_ids: list[str], storage_name: str, bucket_name: str
    ) -> uuid.UUID:

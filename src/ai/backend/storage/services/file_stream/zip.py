...
class ZipArchiveStreamReader(StreamReader):
    ...
    def __init__(self, base_path: Path, entries: list[Path]) -> None:
        self._zf = zipstream.ZipFile(compression=zipstream.ZIP_DEFLATED)
        for file_path in entries:
            arcname = str(PurePosixPath(file_path.relative_to(base_path)))
    ...
    async def read(self) -> AsyncIterator[bytes]:
        ...
        try:
            ...
            while (item := await q.async_q.get()) and not isinstance(item, Sentinel):
                yield item
                q.async_q.task_done()
            await put_chunks
        finally:

...
@dataclass
class StorageHealthProbeArgs:
    valkey_client: AbstractValkeyClient
    etcd: AsyncEtcd
    timeout: float = 5.0


async def create_health_probe(args: StorageHealthProbeArgs) -> HealthProbe:
    """
    Create and configure a HealthProbe for the Storage Proxy component.

    This function creates a HealthProbe instance and registers health checkers
    based on the provided dependencies.

    Args:
        args: Storage health probe configuration arguments

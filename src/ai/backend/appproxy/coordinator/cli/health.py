...
@click.group()
def cli() -> None:
    """Health check commands for app proxy coordinator components."""
    pass
...
def check(
    ...
    """
    Perform health checks on all app proxy coordinator dependencies.

    This command initializes all coordinator dependencies and then performs
    ...
    Examples:
        ...
        $ backend.ai app-proxy-coordinator health check

        # Hide timestamps in output
        $ backend.ai app-proxy-coordinator health check --no-timestamps

        # Set log level to DEBUG to see detailed check logs
        $ backend.ai app-proxy-coordinator health check --log-level DEBUG
    """
    import asyncio
    ...
    async def _initialize_and_check_all_components(
        ...
        """
        Initialize dependencies using CoordinatorDependencyComposer.

        Uses the CoordinatorDependencyComposer to initialize all dependencies in the correct order,
        then collects health checkers from the DependencyBuilderStack.
        """
        from ai.backend.appproxy.coordinator.dependencies.composer import (
        ...
        async with dependency_stack:
            # Use CoordinatorDependencyComposer to initialize all dependencies
            coordinator_composer = CoordinatorDependencyComposer()
    ...
    async def _check_health() -> bool:
        """Perform health checks on all coordinator components."""
        print("\n" + "=" * 60)
        print("App Proxy Coordinator Health Check")
        print("=" * 60)
        ...
        try:
            async with _initialize_and_check_all_components(
                config_path or Path("app-proxy-coordinator.toml"),
                probe,
            ):
                pass

        ...
        if self.enabled:
            ...
            try:
                ...
                for cid in container_ids:
                    # The agent's own allocation, not the container runtime's record of it:
                    # reading Docker's HostConfig.DeviceRequests 404'd for every containerd
                    # container, once per stat cycle, and those kernels reported no GPU use.
                    ...
                    for device_id in device_ids:
                        ...
                        if mem_stat is None or util_stat is None:
                            continue  # masked, or a device NVML no longer reports

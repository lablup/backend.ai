...
from . import __version__, lpu_api
...
class LPUPlugin(AbstractComputePlugin):
        ...
        for dev in await self.list_devices():
            if dev.device_id in device_alloc.get(SlotName(SLOT_NAME), {}).keys():
                assigned_devices.extend(
                    [dev.xvc_pri_path.as_posix(), dev.renderD_path.as_posix()],
                )
    ...
    async def restore_from_container(
        ...
        if hasattr(alloc_map, "apply_allocation"):
            ...
            alloc_map.allocations[SlotName(SLOT_NAME)].update(
    ...
    async def generate_resource_data(
        ...
        data["LPU_GLOBAL_DEVICE_IDS"] = ",".join(
    ...
    def get_metadata(self) -> AcceleratorMetadata:
        return {
            ...
            "human_readable_name": "Hyperaccel LPU Device",
            "display_unit": "Core",

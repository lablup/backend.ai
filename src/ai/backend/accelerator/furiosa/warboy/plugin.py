...
class WarboyPlugin(AbstractComputePlugin):
    key = DeviceName("warboy")
    slot_types: Sequence[Tuple[SlotName, SlotTypes]] = (
        (SlotName("warboy.device"), SlotTypes("count")),
    )
    exclusive_slot_types: Set[str] = {"warboy.device"}
    ...
    async def list_devices(self) -> List[WarboyDevice]:
        ...
        devices: List[WarboyDevice] = []
        cnt = 0
        async for device_info in WarboyAPI.list_devices():
            devices.append(
                WarboyDevice(
    ...
    async def available_slots(self) -> Mapping[SlotName, Decimal]:
        ...
        return {
            SlotName("warboy.device"): Decimal(len(devices)),
        }
    ...
    async def generate_docker_args(
        ...
        device_ids = [
            ...
            if x.device_id in device_alloc.get(SlotName("warboy.device"), {}).keys()
        ]
    ...
    async def get_attached_devices(
        ...
        if SlotName("warboy.device") in device_alloc:
            device_ids.extend(device_alloc[SlotName("warboy.device")].keys())
    ...
    async def restore_from_container(
        ...
        if hasattr(alloc_map, "apply_allocation"):
            ...
            alloc_map.allocations[SlotName("warboy.device")].update(
                resource_spec.allocations.get(
                    DeviceName("warboy"),
                    {},
                ).get(
                    SlotName("warboy.device"),
                    {},
                ),
            )
    ...
    def get_metadata(self) -> AcceleratorMetadata:
        return {
            ...
            "description": "Warboy",
            "human_readable_name": "Furiosa Warboy Device",
            "display_unit": "Core",

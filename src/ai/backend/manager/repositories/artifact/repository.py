...
class ArtifactRevisionFilterApplier(BaseFilterApplier[ArtifactRevisionFilterOptions]):
    ...
    def apply_entity_filters(
        ...
        if filters.version_filter is not None:
            version_filter_data = filters.version_filter.to_dataclass()
            version_condition = version_filter_data.apply_to_column(ArtifactRevisionRow.version)
            if version_condition is not None:
                conditions.append(version_condition)

        # Handle IntFilter-based size filter
        if filters.size_filter is not None:
            size_filter_data = filters.size_filter.to_dataclass()
            size_condition = size_filter_data.apply_to_column(ArtifactRevisionRow.size)

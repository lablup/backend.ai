...
from ai.backend.manager.data.resource_preset.creator import ResourcePresetCreator
from ai.backend.manager.data.resource_preset.modifier import ResourcePresetModifier
from ai.backend.manager.data.resource_preset.types import ResourcePresetData
from ai.backend.manager.errors.resource_preset import ResourcePresetNotFound
...
class ResourcePresetRepository:
    ...
    async def create_preset_validated(self, creator: ResourcePresetCreator) -> ResourcePresetData:
        """
        Creates a new resource preset using ResourcePresetCreator.
        Raises ResourcePresetAlreadyExists if a preset with the same name and scaling group already exists.
        """
        async with self._db.begin_session() as session:
            preset_data = creator.fields_to_store()
            preset_row = ResourcePresetRow(**preset_data)
            session.add(preset_row)
            await session.flush()
    ...
    async def _get_preset_by_id(self, session: SASession, preset_id: UUID) -> ResourcePresetRow:
        """
        Private method to get a preset by ID using an existing session.
        Raises ResourcePresetNotFound if not found.
        """
        preset_row = await session.scalar(
    ...
    async def _get_preset_by_name(self, session: SASession, name: str) -> ResourcePresetRow:
        """

...
class AgentStatefulSource:
    ...
    async def read_agent_container_counts(self, agent_ids: list[AgentId]) -> Mapping[AgentId, int]:
        """Read container count for the given agent IDs."""
        casted_agent_ids = [str(agent_id) for agent_id in agent_ids]
        container_counts = await self._valkey_stat.get_agent_container_counts_as_dict(
            casted_agent_ids
        )
        # Convert string keys back to AgentId
        return {AgentId(agent_id): container_counts[agent_id] for agent_id in casted_agent_ids}

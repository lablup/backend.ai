...
class AppConfigFragmentCreatorSpec(DependentCreatorSpec[int, AppConfigFragmentRow]):
    ...
    ``build_row`` receives the computed next rank as its dependency, so a newly created
    fragment is placed after the existing fragments for the same ``config_name``.
    """
    ...
    @override
    def build_row(self, dependency: int) -> AppConfigFragmentRow:

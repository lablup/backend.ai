...
class AppConfigFragmentDBSource:
    ...
    async def list_visible_fragments_bulk(
        ...
        contributes; a ``scope`` (principal) additionally admits its own domain and user
        overlay. With ``scope=None`` — the anonymous, pre-login read — only ``public``-scope
        documents contribute, so the caller sees the shared baseline with no principal overlay.

...
class ModelServingService:
    ...
    async def create(self, action: CreateModelServiceAction) -> CreateModelServiceActionResult:
        ...
        image_row = await self._repository.resolve_image([
        ...
        if await self._repository.check_service_name_exists(action.creator.service_name):
        ...
        endpoint_id = await self._repository.create_endpoint(endpoint)
    ...
    async def list_serve(self, action: ListModelServiceAction) -> ListModelServiceActionResult:
        rows = await self._repository.list_endpoints(action.session_owener_id, action.name)
    ...
    async def delete(self, action: DeleteModelServiceAction) -> DeleteModelServiceActionResult:
        ...
        endpoint = await self._repository.get_endpoint_by_id(service_id, load_routes=True)
        await self._repository.verify_user_access_scopes(
            action.requester_ctx, endpoint.session_owner
        )

        if len(endpoint.routings) == 0:
            await self._repository.update_endpoint_to_destroyed(service_id)
        else:
            await self._repository.update_endpoint_to_destroying(service_id)
    ...
    async def dry_run(self, action: DryRunModelServiceAction) -> DryRunModelServiceActionResult:
        ...
        created_user = await self._repository.get_user_by_id(action.request_user_id)
        ...
        async def _task(reporter: ProgressReporter) -> None:
            ...
            async def _handle_event(
                ...
                match event:
                    ...
                    case ModelServiceStatusBroadcastEvent():
                        session = await self._repository.get_session_with_kernels(
                            result["sessionId"]
                        )
    ...
    async def clear_error(self, action: ClearErrorAction) -> ClearErrorActionResult:
        ...
        await self._repository.delete_failed_routes(action.service_id)
        await self._repository.reset_endpoint_retries(endpoint.id)

        return ClearErrorActionResult(success=True)

    async def update_route(self, action: UpdateRouteAction) -> UpdateRouteActionResult:
        route = await self._repository.get_route_by_id(action.route_id, load_endpoint=True)
        ...
        await self._repository.update_route_traffic_ratio(action.route_id, action.traffic_ratio)

        endpoint = await self._repository.get_endpoint_with_routes(action.service_id)
        async with self._repository.get_db_session_for_agent_registry() as db_sess:
            try:
    ...
    async def delete_route(self, action: DeleteRouteAction) -> DeleteRouteActionResult:
        route = await self._repository.get_route_by_id(
            action.route_id, load_endpoint=True, load_session=True
        )
        if route.endpoint != action.service_id:
            raise RouteNotFound
        await self._repository.verify_user_access_scopes(
        ...
        await self._repository.update_endpoint_replicas(
            action.service_id, route.endpoint_row.replicas - 1
        )
    ...
    async def generate_token(self, action: GenerateTokenAction) -> GenerateTokenActionResult:
        endpoint = await self._repository.get_endpoint_by_id(action.service_id, load_routes=True)
        sgroup_info = await self._repository.get_scaling_group_wsproxy_info(endpoint.resource_group)
        wsproxy_addr = sgroup_info["wsproxy_addr"]
        wsproxy_api_token = sgroup_info["wsproxy_api_token"]
        ...
        token_row = await self._repository.create_endpoint_token(token_row)
    ...
    async def modify_endpoint(self, action: ModifyEndpointAction) -> ModifyEndpointActionResult:
        async def _do_mutate() -> MutationResult:
            try:
                endpoint_row = await self._repository.get_endpoint_with_all_relations(
                    action.endpoint_id,
            ...
            if image_ref is not None:
                ...
                image_row = await self._repository.resolve_image([
                    ImageIdentifier(image_name, arch),
            ...
            async with self._repository.get_db_session_for_business_logic() as db_session:
            ...
            await self._repository.commit_endpoint_changes(endpoint_row)

...
class AuthService:
    ...
    async def get_role(self, action: GetRoleAction) -> GetRoleActionResult:
        ...
        if action.group_id is not None:
            ...
            row = await self._repository.user.check_user_in_group(action.group_id, action.user_id)
            if row is None:
    ...
    async def authorize(self, action: AuthorizeAction) -> AuthorizeActionResult:
        ...
        if hook_result.status != PASSED:
            ...
            user = await self._repository.user.check_user_credential(
        ...
        user_row = await self._repository.user.get_user_row_by_uuid(user["uuid"])
        if user_row is None:
    ...
    async def signup(self, action: SignupAction) -> SignupActionResult:
        ...
        existing_user = await self._repository.user.check_email_exists(action.email)
        ...
        user = await self._repository.user.create_user(data)
        if user is None:
        ...
        await self._repository.keypair.create_keypair(kp_data)

        # Add user to the default group.
        group_name = user_data_overriden.get("group", "default")
        await self._repository.user.add_user_to_group(user.uuid, group_name, action.domain_name)
    ...
    async def signout(self, action: SignoutAction) -> SignoutActionResult:
        ...
        email = action.email
        result = await self._repository.user.check_user_credential(
            domain=action.domain_name,
            email=email,
            password=action.password,
        )
        ...
        # Inactivate the user.
        await self._repository.user.update_user_status(email, UserStatus.INACTIVE)
        # Inactivate every keypairs of the user.
        await self._repository.keypair.deactivate_user_keypairs(email)
    ...
    async def update_full_name(self, action: UpdateFullNameAction) -> UpdateFullNameActionResult:
        email = action.email
        domain_name = action.domain_name
        user = await self._repository.user.get_user_by_email_and_domain(email, domain_name)
        if user is None:
            return UpdateFullNameActionResult(success=False)

        # If user is not null, then it updates user full_name.
        await self._repository.user.update_user_full_name(email, action.full_name)
    ...
    async def update_password(self, action: UpdatePasswordAction) -> UpdatePasswordActionResult:
        ...
        await self._repository.user.update_user_password(email, action.new_password)
    ...
    async def update_password_no_auth(
        ...
        checked_user = await self._repository.user.check_user_credential(
            domain=action.domain_name,
            email=action.email,
        ...
        changed_at = await self._repository.user.update_user_password_by_uuid(
    ...
    async def generate_ssh_keypair(
        ...
        await self._repository.keypair.update_ssh_keypair(action.access_key, pubkey, privkey)
    ...
    async def _check_password_age(self, user: Row, auth_config: Optional[AuthConfig]) -> None:
        if (
            ...
            current_dt: datetime = await self._repository.user.get_current_datetime()

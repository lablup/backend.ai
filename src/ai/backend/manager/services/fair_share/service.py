...
class FairShareService:
    ...
    async def get_domain_fair_share(
        ...
        try:
            ...
            default_data = DomainFairShareData(
                ...
                uses_default_resources=[],  # No defaults used (full resource_weights from spec)
            )
    ...
    async def search_rg_domain_fair_shares(
        ...
        Repository creates default fair share data for entities without records.
        """
        # Repository now returns complete FairShareData for all domains
        result = await self._repository.search_domain_fair_shares_by_scope(
    ...
    async def search_rg_project_fair_shares(
        ...
        Repository creates default fair share data for entities without records.
        """
        # Repository now returns complete FairShareData for all projects
        result = await self._repository.search_project_fair_shares_by_scope(
            action.scope, action.querier
        )
    ...
    async def search_rg_user_fair_shares(
        ...
        Repository creates default fair share data for entities without records.
        """
        # Repository now returns complete FairShareData for all users
        result = await self._repository.search_user_fair_shares_by_scope(
            action.scope, action.querier
        )

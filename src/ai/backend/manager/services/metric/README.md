...
| `pct` | Computed in PromQL | `Σcurrent / Σcapacity × 100` over the scope's kernels; `current` is wrapped in `rate()` when the metric's unit hint is `millicores` (the stored series is a cumulative CPU-time counter). Metrics without a capacity series return no data |

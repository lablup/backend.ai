...
class ProjectResourcePolicyService:
    ...
    async def get_project_resource_policy(
        ...
        data = await self._project_resource_policy_repository.get_by_name(action.name)
        return GetProjectResourcePolicyActionResult(data=data)

    async def search_project_resource_policies(
        self, action: SearchProjectResourcePoliciesAction
    ) -> SearchProjectResourcePoliciesActionResult:
        result = await self._project_resource_policy_repository.search(querier=action.querier)

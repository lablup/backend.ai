from __future__ import annotations

from collections.abc import Sequence
from dataclasses import dataclass, replace
from typing import Self, override

from ai.backend.common.data.entity.idle_checker import IdleCheckerID
from ai.backend.common.data.entity.types import EntityIdentifier
from ai.backend.manager.actions.v2.ops.base import PartialBulkGetEntityOpsAction
from ai.backend.manager.data.idle_checker.types import IdleCheckerData
from ai.backend.manager.models.idle_checker.queriers import BulkIdleCheckerQuerier
from ai.backend.manager.models.idle_checker.row import IdleCheckerRow


@dataclass
class BulkGetIdleCheckersAction(PartialBulkGetEntityOpsAction[IdleCheckerRow, IdleCheckerData]):
    """Read the idle checkers the caller named, answering for each id."""

    ids: Sequence[IdleCheckerID]

    @override
    @classmethod
    def action_name(cls) -> str:
        return "bulk_get_idle_checkers"

    @override
    def entity_ids(self) -> Sequence[EntityIdentifier]:
        return tuple(self.ids)

    @override
    def to_querier(self) -> BulkIdleCheckerQuerier:
        return BulkIdleCheckerQuerier()

    @override
    def narrowed_to(self, entity_ids: Sequence[EntityIdentifier]) -> Self:
        allowed = frozenset(entity_ids)
        return replace(self, ids=[entity_id for entity_id in self.ids if entity_id in allowed])

---
name: group-service-composition
type: decision-table
description: 프로젝트 처리기의 모양 선택, dotfile이 프로젝트 컬럼인 이유, 멤버십이 별도 경로인 이유
scope: src/ai/backend/manager/services/group
keywords:
  - GroupProcessors
  - GroupCreator
  - GroupUpdater
...
패키지 이름은 `group`이고 안쪽 이름은 project다. 도메인 아래에 놓이므로 생성은 도메인을
scope로 하는 scope 모양이고, 이미 만들어진 프로젝트를 지목하는 연산은 single_entity다.
...
| `create_group` | scope (도메인) | CREATE |
| `delete_group` / `restore_group` | single_entity | DELETE / RESTORE |
| `update_group` | single_entity | UPDATE |
| `purge_group` | single_entity | PURGE |
...
`update_group`이 멤버십을 함께 바꿀 때 멤버십 쓰기와 행 갱신은 서로 다른 트랜잭션이다.

## dotfile은 프로젝트 행의 컬럼이다

`groups.dotfiles`에 msgpack으로 묶여 저장된다. 판정은 `data/dotfile/types.py`의
...
| 연산 | 남은 이유 |
|---|---|
| `update_group` | 멤버십 변경을 함께 받는다 |
| `purge_group` | vfolder와 세션을 함께 정리한다 |

    ...
    async def _prepare_kernels(
        ...
        for idx, kernel_config in enumerate(kernel_configs):
            ...
            kernel_data = KernelEnqueueData(
                ...
                uid=kernel_config.get("uid") or context.user_container_info.uid,
                main_gid=kernel_config.get("main_gid") or context.user_container_info.main_gid,
                gids=kernel_config.get("supplementary_gids")
                or context.user_container_info.supplementary_gids,
            )

"""Rolling update FSM evaluation for a single deployment cycle (BEP-1049).
...
def rolling_update_evaluate(
    ...
    if total_new_live == 0 and new_failed:
        ...
        return CycleEvaluationResult(sub_step=DeploymentSubStep.ROLLED_BACK)
    ...
    if to_create > 0:
        route_changes.scale_out_specs = _build_route_creators(deployment, to_create)
    ...
    if to_terminate > 0:
        ...
        for r in sorted_old[:to_terminate]:
            route_changes.scale_in_route_ids.append(r.route_id)

...
Every declared entity is created in the `global` scope as well.
...
async def provision_fixture_entities(conn: AsyncConnection, table_names: Collection[str]) -> None:
    ...
    that owns and governs it, owned and governed by the `global` scope too, as creating
    the row through the entity ops would have.
...
async def _provision_table(conn: AsyncConnection, table_name: str, spec: FixtureEntitySpec) -> None:
    ...
    global_nodes = (
        ...
        .join(GlobalEntityRow, VirtualEntityRow.entity_id == GlobalEntityRow.id)
        .where(
            VirtualEntityRow.entity_type == GlobalEntityType(),
            GlobalEntityRow.name == GlobalEntityName.GLOBAL,
        )
        .subquery()
    )
    members = nodes.subquery()
    pairs = sa.select(global_nodes.c.id.label("scope"), members.c.id.label("member")).select_from(
        global_nodes.join(members, sa.true())
    )
    await conn.execute(
        pg_insert(EntityMembershipRow)
        .from_select(
            ["virtual_entity_id", "member_entity_id", "capped"],
            pairs.add_columns(sa.literal(False)),
        )
        .on_conflict_do_nothing(index_elements=["virtual_entity_id", "member_entity_id"])
    )
    await conn.execute(
        ...
        .from_select(
            ["virtual_entity_id", "scope_entity_id"],
            sa.select(members.c.id, global_nodes.c.id).select_from(
                global_nodes.join(members, sa.true())
            ),
        )
        .on_conflict_do_nothing()

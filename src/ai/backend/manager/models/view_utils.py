...
def create_table_from_selectable(
    ...
    if aliases is None:
        aliases = {}
    args = [
        sa.Column(c.name, c.type, key=aliases.get(c.name, c.name), primary_key=c.primary_key)
...
def create_materialized_view(name, selectable, metadata, indexes=None, aliases=None):
    ...
    table = create_table_from_selectable(
        name=name, selectable=selectable, indexes=indexes, metadata=None, aliases=aliases
    )

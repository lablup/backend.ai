...
TCreateSpec = TypeVar("TCreateSpec", bound=CreateSpec)
TUpdateSpec = TypeVar("TUpdateSpec", bound=UpdateSpec)
...
class ABCDeclarativeMeta(ABCMeta, type(Base)):  # type: ignore[misc]
    """Combined metaclass for ABC + SQLAlchemy DeclarativeBase"""

    pass


class BaseRow(Base, Generic[TCreateSpec, TUpdateSpec], metaclass=ABCDeclarativeMeta):
    __abstract__ = True

    @classmethod
    @abstractmethod
    def build(cls, spec: TCreateSpec) -> Self:
    ...
    @classmethod
    async def create(cls, db_sess: SASession, spec: TCreateSpec) -> Self:
        row = cls.build(spec)
        db_sess.add(row)
        await db_sess.flush()
        return row
    ...
    @classmethod
    async def query(cls, db_sess: SASession, querier: Querier) -> Self | None:
        ...
        stmt = sa.select(cls).where(pk_columns[0] == querier.pk_value)

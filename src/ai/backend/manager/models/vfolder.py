...
class PermissionContextBuilder(
    ...
    async def _build_in_user_scope(
        ...
        user_id_to_permission_map: Mapping[uuid.UUID, frozenset[VFolderRBACPermission]] = {}
    ...
    async def _build_in_project_scope(
        ...
        role_in_project = await ctx.get_user_role_in_project(db_session, project_id)
    ...
    async def _build_in_domain_scope(
        ...
        user_id_to_permission_map: Mapping[uuid.UUID, frozenset[VFolderRBACPermission]] = {}
        project_id_to_permission_map: Mapping[uuid.UUID, frozenset[VFolderRBACPermission]] = {}
        domain_name_to_permission_map: Mapping[str, frozenset[VFolderRBACPermission]] = {}
        object_id_to_additional_permission_map: Mapping[
            uuid.UUID, frozenset[VFolderRBACPermission]
        ] = {}
        object_id_to_overriding_permission_map: Mapping[
...
async def validate_permission(
    ...
    *,
    ...
    async with ctx.db.begin_readonly_session(db_conn) as db_session:
        permission_ctx = await PermissionContextBuilder.build(
            db_session, ctx, target_scope, permission=permission
        )
        query_stmt = await permission_ctx.build_query()
        if query_stmt is None:

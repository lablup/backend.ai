...
from ai.backend.common.types import VfHostPermissionMap, VFolderMount
...
async def get_allowed_vfolder_hosts_by_group(
    ...
    if values := await conn.scalar(query):
        allowed_hosts = allowed_hosts.union(values)
    ...
    if group_id is not None:
        ...
        if rows := (await conn.execute(query)).fetchall():
            for row in rows:
                allowed_hosts = allowed_hosts.union(row.allowed_vfolder_hosts)
    # Keypair Resource Policy's allowed_vfolder_hosts
    allowed_hosts = allowed_hosts.union(resource_policy["allowed_vfolder_hosts"])

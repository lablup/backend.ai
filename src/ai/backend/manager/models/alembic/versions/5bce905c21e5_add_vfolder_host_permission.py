...
# Private MetaData — the app-wide `metadata` already has today's columns
# registered, which extend_existing=True would silently reuse instead of
# this migration's own point-in-time snapshot.
...
def upgrade() -> None:
    ...
    def get_pk_vfolder_host_maps(
        query: sa.sql.Select[Any], pk_name: str
    ...
    def update_vfolder_host_col(
        table: sa.Table, cond: sa.ColumnElement[Any], row_list: list[dict[str, object]]
    ) -> None:
        query = (
            sa.update(table)

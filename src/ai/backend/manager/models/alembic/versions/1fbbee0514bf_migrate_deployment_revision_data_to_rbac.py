...
def _migrate_new_entity_type(db_conn: Connection) -> None:
    """Add DEPLOYMENT_REVISION entity type permissions to all permission groups."""
    offset = 0
        ...
        for perm_group in perm_groups:
            inputs.extend(
                _add_entity_typed_permission_to_permission_groups(
                    perm_group,
                    EntityType.DEPLOYMENT_REVISION,
                )
            )
...
def downgrade() -> None:
    ...
    _remove_entity_type_permissions(conn, EntityType.DEPLOYMENT_REVISION)

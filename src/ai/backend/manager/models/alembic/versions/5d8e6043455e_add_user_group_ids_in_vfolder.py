...
def upgrade():
    ...
    op.create_foreign_key(op.f('fk_vfolders_user_users'),
                          'vfolders', 'users', ['user'], ['uuid'])
    op.create_foreign_key(op.f('fk_vfolders_group_groups'),
                          'vfolders', 'groups', ['group'], ['id'])
    op.add_column('vfolder_permissions', sa.Column('user', GUID(), nullable=True))
    op.create_foreign_key(op.f('fk_vfolder_permissions_user_users'),
                          'vfolder_permissions', 'users', ['user'], ['uuid'])
    ...
    j = vfolder_permissions.join(keypairs,
                                 vfolder_permissions.c.access_key == keypairs.c.access_key)
    query = (sa.select([vfolder_permissions.c.vfolder, keypairs.c.access_key, keypairs.c.user])
               .select_from(j))
    results = connection.execute(query).fetchall()
    ...
    op.drop_constraint('fk_vfolder_permissions_access_key_keypairs',
                       'vfolder_permissions', type_='foreignkey')
...
def downgrade():
    ...
    op.add_column('vfolders',
                  sa.Column('belongs_to', sa.String(length=20), autoincrement=False, nullable=True))
    op.create_foreign_key('fk_vfolders_belongs_to_keypairs',
                          'vfolders', 'keypairs', ['belongs_to'], ['access_key'])
    op.add_column('vfolder_permissions',
    ...
    op.create_foreign_key('fk_vfolder_permissions_access_key_keypairs',
                          'vfolder_permissions', 'keypairs', ['access_key'], ['access_key'])
    ...
    query = (sa.select([vfolder_permissions.c.vfolder, keypairs.c.user, keypairs.c.access_key])
               .select_from(j))
    results = connection.execute(query).fetchall()
    updates = [{'_vfolder': row.vfolder, '_access_key': row.access_key, '_user': row.user} \
               for row in results]

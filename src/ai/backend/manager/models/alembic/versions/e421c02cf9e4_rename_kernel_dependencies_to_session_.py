...
def upgrade():
    ...
    op.execute("ALTER INDEX pk_kernel_dependencies " "RENAME TO pk_session_dependencies")
    op.execute(
        "ALTER INDEX ix_kernel_dependencies_depends_on "
        "RENAME TO ix_session_dependencies_depends_on"
    )
    op.execute(
        "ALTER INDEX ix_kernel_dependencies_kernel_id "
...
def downgrade():
    ...
    op.execute("ALTER INDEX pk_session_dependencies " "RENAME TO pk_kernel_dependencies")
    op.execute(
        "ALTER INDEX ix_session_dependencies_depends_on "
        "RENAME TO ix_kernel_dependencies_depends_on"
    )
    op.execute(
        "ALTER INDEX ix_session_dependencies_session_id "

...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            type_=sa.Enum(*default_for_unspecified_choices, name="default_for_unspecified"),
            nullable=False,
        ),
        ...
        sa.Column("allowed_vfolder_hosts", postgresql.ARRAY(sa.String()), nullable=False),
...
def downgrade():
    ...
    op.add_column(
        "keypairs", sa.Column("concurrency_limit", sa.INTEGER(), autoincrement=False, nullable=True)
    )
    op.alter_column(
        "keypairs",
        "resource_policy",

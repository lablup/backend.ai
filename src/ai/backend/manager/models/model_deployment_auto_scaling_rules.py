...
def _get_auto_scaling_rule_deployment_join_cond():
    ...
    return ModelDeploymentAutoScalingRuleRow.model_deployment_id == foreign(ModelDeploymentRow.id)
...
class ModelDeploymentAutoScalingRuleRow(Base):
    ...
    __table_args__ = (
        # Constraint: min_threshold or max_threshold - only one must be set
        sa.CheckConstraint(
            "(min_threshold IS NOT NULL AND max_threshold IS NULL) OR "
            "(min_threshold IS NULL AND max_threshold IS NOT NULL)",
            name="check_single_threshold",
        ),
        # Constraint: unique model_deployment_id + metric_name combination
        sa.UniqueConstraint(

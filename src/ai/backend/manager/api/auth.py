...
async def auth_middleware(request: web.Request, handler) -> web.StreamResponse:
    ...
    if hook_result.status != PASSED:
        ...
        if access_key is not None:
            ...
            now_result = await root_ctx.valkey_stat.time()
            now = now_result[0] + (now_result[1] / (10**6))

            # Use ValkeyStatClient individual operations
            num_queries_key = f"kp:{access_key}:num_queries"
            await root_ctx.valkey_stat.incr(num_queries_key)
            await root_ctx.valkey_stat.expire(num_queries_key, 86400 * 30)  # retention: 1 month
            last_call_time_key = f"kp:{access_key}:last_call_time"
            await root_ctx.valkey_stat.set(last_call_time_key, str(now).encode())
            await root_ctx.valkey_stat.expire(last_call_time_key, 86400 * 30)  # retention: 1 month
        else:

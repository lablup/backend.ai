"""Backward-compatibility shim for the quota_scope module.

All quota scope handler logic has been migrated to:

* ``api.rest.quota_scope.handler`` — QuotaScopeHandler (constructor DI)
* ``api.rest.quota_scope`` — register_routes()

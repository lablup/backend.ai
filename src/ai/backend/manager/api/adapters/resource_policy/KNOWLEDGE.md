...
키페어 정책의 `admin_create_keypair_resource_policy`, `admin_get_keypair_resource_policy`,
`admin_search_keypair_resource_policies`, `admin_update_keypair_resource_policy`,
`admin_delete_keypair_resource_policy`, `get_my_keypair_resource_policy`는 모두 위에 적혀 있다.
프로젝트 정책과 사용자 정책의 호출은 아직 절이 없다. 실행 결과가 그 목록을 함께 출력한다.

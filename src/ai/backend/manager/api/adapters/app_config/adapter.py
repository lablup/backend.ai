...
class AppConfigAdapter(BaseAdapter):
        ...
        only_if = ExistsQuerier(
            ...
            conditions=[
                AppConfigAllowListConditions.by_config_name_equals(
                    StringMatchSpec(spec.config_name, case_insensitive=False, negated=False)
                ),
                AppConfigAllowListConditions.by_scope_type_equals(spec.scope_type),
            ],
        )
        action_result = await self._processors.app_config_fragment.create.wait_for_complete(
            CreateAppConfigFragmentAction(creator_spec=spec, only_if=only_if)
        )
    ...
    async def admin_update(
        ...
        # The target fragment's (config_name, scope_type) are unknown here, so the gate
        # matches the allow-list against the target row via correlated subqueries.
        only_if = ExistsQuerier(
            row_class=AppConfigAllowListRow,
            conditions=[
                lambda: AppConfigAllowListRow.config_name
                == (
                    sa.select(AppConfigFragmentRow.config_name)
                    .where(AppConfigFragmentRow.id == fragment_id)
                    .scalar_subquery()
                ),
                lambda: AppConfigAllowListRow.scope_type
                == (
                    sa.select(AppConfigFragmentRow.scope_type)
                    .where(AppConfigFragmentRow.id == fragment_id)
                    .scalar_subquery()
                ),
            ],
        ...
        action_result = await self._processors.app_config_fragment.update.wait_for_complete(
            UpdateAppConfigFragmentAction(updater=updater, only_if=only_if)
        )
    ...
    async def admin_purge(
        ...
        action_result = await self._processors.app_config_fragment.purge.wait_for_complete(
            PurgeAppConfigFragmentAction(purger=purger, only_if=only_if)
        )
    ...
    async def resolve(self, input: ResolveAppConfigInput) -> ResolveAppConfigPayload:
        ...
        action_result = await self._processors.app_config.resolve.wait_for_complete(
            ResolveAppConfigAction(
                config_name=input.config_name,
                domain_id=DomainID(input.domain_id),
    ...
    async def resolve_public(self, config_name: str) -> ResolveAppConfigPayload:
        action_result = await self._processors.app_config.resolve_public.wait_for_complete(
            ResolvePublicAppConfigAction(config_name=config_name)
        )
        return ResolveAppConfigPayload(
            app_config=self._app_config_to_node(action_result.app_config)
        )
    ...
    def _convert_orders(self, orders: list[AppConfigFragmentOrder]) -> list[QueryOrder]:
        ...
        for order in orders:
            ...
            match order.field:
                ...
                case AppConfigFragmentOrderField.RANK:
                    result.append(AppConfigFragmentOrders.rank(ascending))

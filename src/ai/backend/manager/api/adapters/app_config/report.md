...
같은 키에 목록을 담은 두 조각을 읽으면, 뒤엣것의 목록만 남고 이어 붙지 않는다

Given

- 도메인·사용자 스코프에 열린 이름 하나와, 자기 스코프에서 설정 읽기 권한을 받은 사용자 한 명
  - 도메인 home-1
  - 자기 스코프에서 설정에 READ 권한을 받은 사용자 준비
    - 도메인에 속한 사용자 한 명 준비
  ...
  - 조각 domain-fragment-1: 값 {'plugins': ['git', 'lint']}
  - 조각 own-fragment-1: 값 {'plugins': ['spell']}
...
같은 이름을 두 번 적어 읽으면, 같은 값이 두 번 온다

Given

- 공개 스코프에 열린 이름 하나와, 자기 스코프에서 설정 읽기 권한을 받은 사용자 한 명
  - 도메인 home-1
  - 자기 스코프에서 설정에 READ 권한을 받은 사용자 준비
    - 도메인에 속한 사용자 한 명 준비
      - 프로젝트 정책 default: 사용자를 만들 때 딸려 만들어지는 개인 프로젝트가 이 이름으로 찾는다
  ...
  - 공개 조각 public-fragment-1: 값 {'theme': 'light', 'menu': {'home': True, 'docs': True}}
...
정의만 있고 허용 항목도 조각도 없는 이름을 읽기 권한을 받은 사용자가 읽으면, 그 이름이 답에서 빠지지 않고 빈 설정으로 온다

Given

- 아무 스코프에도 열리지 않은 스코프에 열린 이름 하나와, 자기 스코프에서 설정 읽기 권한을 받은 사용자 한 명
  - 도메인 home-1
  - 자기 스코프에서 설정에 READ 권한을 받은 사용자 준비
    - 도메인에 속한 사용자 한 명 준비
      - 프로젝트 정책 default: 사용자를 만들 때 딸려 만들어지는 개인 프로젝트가 이 이름으로 찾는다
...
정의조차 없는 이름을 읽기 권한을 받은 사용자가 읽으면, 정의가 없다고 거부되지 않고 빈 설정으로 온다. 이 읽기는 정의를 보지 않고 조각만 본다

Given

- 등록되지 않은 이름과, 자기 스코프에서 설정 읽기 권한을 받은 사용자 한 명
  - 도메인 home-1
  - 자기 스코프에서 설정에 READ 권한을 받은 사용자 준비
    - 도메인에 속한 사용자 한 명 준비
      - 프로젝트 정책 default: 사용자를 만들 때 딸려 만들어지는 개인 프로젝트가 이 이름으로 찾는다
...
공개 스코프에만 조각이 있는 이름을 읽으면, 그 조각의 값이 그대로 온다
...
도메인 허용 항목의 순위를 사용자 항목보다 크게 두면, 같은 조각들에서 겹치는 키의 승자가 도메인으로 뒤바뀐다. 순위는 허용 항목에 있고 값의 소유자는 바꿀 수 없다
...
- 도메인·사용자 스코프에 열린 이름 하나와, 자기 스코프에서 설정 읽기 권한을 받은 사용자 한 명
  ...
  - 도메인·사용자 스코프에 열린 이름 준비
    - 설정 정의 config-1: 이 이름의 설정이 등록돼 있다
    - 허용 항목 entry-1: 도메인 스코프가 이 이름을 채울 수 있다, 순위 400
    - 허용 항목 entry-2: 사용자 스코프가 이 이름을 채울 수 있다, 순위는 그 종류의 기본값
  - 조각 domain-fragment-1: 값 {'theme': 'dark', 'menu': {'docs': False, 'billing': True}}
  - 조각 own-fragment-1: 값 {'theme': 'solar', 'menu': {'home': False}}
...
아무 권한도 받지 않은 사용자가 자기 설정을 읽으면, 스코프 권한 문에서 권한 부족으로 거부된다

Given

- 공개 스코프에 열린 이름 하나와, 설정 권한을 하나도 받지 않은 사용자 한 명
  - 도메인 home-1
  - 설정 권한을 하나도 받지 않은 사용자 준비
    - 도메인에 속한 사용자 한 명 준비
      - 프로젝트 정책 default: 사용자를 만들 때 딸려 만들어지는 개인 프로젝트가 이 이름으로 찾는다
      - 사용자 정책 user-policy-1: 사용자 한 명당 폴더 10개까지
      - 키페어 정책 keypair-policy-1: 동시 세션 5개까지
...
앞 조각의 키에 값이 있고 뒤 조각이 같은 키를 비워 두면, 그 키는 비어 있는 채로 온다. 빈 값도 덮는다
...
- 도메인·사용자 스코프에 열린 이름 하나와, 자기 스코프에서 설정 읽기 권한을 받은 사용자 한 명
  ...
  - 도메인·사용자 스코프에 열린 이름 준비
    ...
    - 허용 항목 entry-1: 도메인 스코프가 이 이름을 채울 수 있다, 순위는 그 종류의 기본값
    - 허용 항목 entry-2: 사용자 스코프가 이 이름을 채울 수 있다, 순위는 그 종류의 기본값
  - 조각 domain-fragment-1: 값 {'banner': 'welcome'}
  - 조각 own-fragment-1: 값 {'banner': None}
...
다른 도메인이 같은 이름에 조각을 두었어도, 자기 설정을 읽으면 자기 도메인 조각의 값만 온다

Given

- 도메인 스코프에 열린 이름 하나와, 자기 스코프에서 설정 읽기 권한을 받은 사용자 한 명
  - 도메인 home-1
  - 자기 스코프에서 설정에 READ 권한을 받은 사용자 준비
    - 도메인에 속한 사용자 한 명 준비
      - 프로젝트 정책 default: 사용자를 만들 때 딸려 만들어지는 개인 프로젝트가 이 이름으로 찾는다
  ...
  - 조각 domain-fragment-1: 값 {'theme': 'dark', 'menu': {'docs': False, 'billing': True}}
  - 도메인 away-1
  - 조각 other-domains-fragment-1: 값 {'theme': 'elsewhere'}
...
같은 도메인의 다른 사용자가 같은 이름에 조각을 두었어도, 자기 설정을 읽으면 자기 조각의 값만 온다

Given

- 사용자 스코프에 열린 이름 하나와, 자기 스코프에서 설정 읽기 권한을 받은 사용자 한 명
  - 도메인 home-1
  - 자기 스코프에서 설정에 READ 권한을 받은 사용자 준비
    - 도메인에 속한 사용자 한 명 준비
      - 프로젝트 정책 default: 사용자를 만들 때 딸려 만들어지는 개인 프로젝트가 이 이름으로 찾는다
  ...
  - 조각 anothers-fragment-1: 값 {'theme': 'theirs', 'secret': 1}
...
같은 이름에 도메인 조각과 자기 조각이 모두 있고 허용 항목이 기본 순위이면, 겹치는 키는 자기 값이 남고 겹치지 않는 키는 양쪽 모두 남는다
...
같은 키 아래 중첩 사전을 담은 두 조각을 읽으면, 겹치지 않는 안쪽 키는 양쪽 모두 남고 겹치는 안쪽 키만 뒤엣것이 남는다
...
- 도메인·사용자 스코프에 열린 이름 하나와, 자기 스코프에서 설정 읽기 권한을 받은 사용자 한 명
  ...
  - 도메인·사용자 스코프에 열린 이름 준비
    ...
    - 허용 항목 entry-1: 도메인 스코프가 이 이름을 채울 수 있다, 순위는 그 종류의 기본값
    - 허용 항목 entry-2: 사용자 스코프가 이 이름을 채울 수 있다, 순위는 그 종류의 기본값
  - 조각 domain-fragment-1: 값 {'editor': {'font': {'size': 12, 'family': 'mono'}, 'wrap': True}}
  - 조각 own-fragment-1: 값 {'editor': {'font': {'size': 14}, 'theme': 'night'}}
...
이름 셋을 한 번에 읽으면, 각각의 병합 결과가 요청한 순서대로 온다

Given

- 공개 조각을 하나씩 가진 이름 3개와, 자기 스코프에서 설정 읽기 권한을 받은 사용자 한 명
  - 도메인 home-1
  - 자기 스코프에서 설정에 READ 권한을 받은 사용자 준비
    - 도메인에 속한 사용자 한 명 준비
      - 프로젝트 정책 default: 사용자를 만들 때 딸려 만들어지는 개인 프로젝트가 이 이름으로 찾는다
  ...
  - 공개 조각 public-fragment-1: 값 {'n': 1}
  - 공개 조각 public-fragment-2: 값 {'n': 2}
  - 공개 조각 public-fragment-3: 값 {'n': 3}
...
같은 이름에 공개 조각과 도메인 조각이 모두 있고 허용 항목이 기본 순위이면, 겹치는 키는 도메인 값이 남는다

Given

- 공개·도메인 스코프에 열린 이름 하나와, 자기 스코프에서 설정 읽기 권한을 받은 사용자 한 명
  - 도메인 home-1
  - 자기 스코프에서 설정에 READ 권한을 받은 사용자 준비
    - 도메인에 속한 사용자 한 명 준비
      - 프로젝트 정책 default: 사용자를 만들 때 딸려 만들어지는 개인 프로젝트가 이 이름으로 찾는다
...
엔티티 권한 집행을 끄면 아무 권한도 받지 않은 사용자도 자기 설정을 읽는다. 이 문은 역할이 아니라 권한 그래프가 지키므로 스위치가 통한다
...
공개·도메인·사용자 조각이 모두 있는 이름을 로그인 없이 읽으면, 공개 조각의 값만 온다. 도메인과 사용자 조각은 섞이지 않는다

Given

- 공개·도메인·사용자 스코프에 열린 이름 하나와, 설정 권한을 하나도 받지 않은 사용자 한 명
  - 도메인 home-1
  - 설정 권한을 하나도 받지 않은 사용자 준비
    - 도메인에 속한 사용자 한 명 준비
      - 프로젝트 정책 default: 사용자를 만들 때 딸려 만들어지는 개인 프로젝트가 이 이름으로 찾는다
      ...
      - 키페어 정책 keypair-policy-1: 동시 세션 5개까지
      - 일반 사용자 user-1: 자기 키와 개인 프로젝트를 갖는다
  - 공개·도메인·사용자 스코프에 열린 이름 준비
    - 설정 정의 config-1: 이 이름의 설정이 등록돼 있다
    - 허용 항목 entry-1: 공개 스코프가 이 이름을 채울 수 있다, 순위는 그 종류의 기본값
    - 허용 항목 entry-2: 도메인 스코프가 이 이름을 채울 수 있다, 순위는 그 종류의 기본값
    - 허용 항목 entry-3: 사용자 스코프가 이 이름을 채울 수 있다, 순위는 그 종류의 기본값
  - 공개 조각 public-fragment-1: 값 {'theme': 'light', 'menu': {'home': True}}
  - 조각 domain-fragment-1: 값 {'theme': 'dark'}
  - 조각 own-fragment-1: 값 {'theme': 'solar'}
...
사용자 조각만 있는 이름을 로그인 없이 읽으면, 빈 설정이 온다

Given

- 사용자 스코프에 열린 이름 하나와, 설정 권한을 하나도 받지 않은 사용자 한 명
  - 도메인 home-1
  - 설정 권한을 하나도 받지 않은 사용자 준비
    - 도메인에 속한 사용자 한 명 준비
      - 프로젝트 정책 default: 사용자를 만들 때 딸려 만들어지는 개인 프로젝트가 이 이름으로 찾는다
...
정의조차 없는 이름을 로그인 없이 읽으면, 공개 조각이 없는 이름과 같은 빈 설정이 온다. 이 읽기로는 어떤 이름이 등록돼 있는지 알 수 없다

Given

- 등록되지 않은 이름과, 설정 권한을 하나도 받지 않은 사용자 한 명
  - 도메인 home-1
  - 설정 권한을 하나도 받지 않은 사용자 준비
    - 도메인에 속한 사용자 한 명 준비
      - 프로젝트 정책 default: 사용자를 만들 때 딸려 만들어지는 개인 프로젝트가 이 이름으로 찾는다
...
같은 조각 셋을 아무 권한도 없는 사용자가 로그인한 채 공개 읽기로 읽어도, 공개 조각의 값만 온다. 이 읽기는 호출자를 아예 보지 않는다
...
이름 셋을 한 번에 로그인 없이 읽으면, 각각의 공개 값이 요청한 순서대로 온다

Given

- 공개 조각을 하나씩 가진 이름 3개와, 설정 권한을 하나도 받지 않은 사용자 한 명
  - 도메인 home-1
  - 설정 권한을 하나도 받지 않은 사용자 준비
    - 도메인에 속한 사용자 한 명 준비
      - 프로젝트 정책 default: 사용자를 만들 때 딸려 만들어지는 개인 프로젝트가 이 이름으로 찾는다

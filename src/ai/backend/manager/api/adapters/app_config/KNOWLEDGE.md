...
| 중첩된 `config`는 안쪽 키까지 병합된다 | 같은 키 아래 중첩 `dict`를 담은 두 `fragment`, 읽기 권한 있음 | 조회 | 겹치지 않는 안쪽 키는 양쪽 모두 남고, 겹치는 안쪽 키는 병합 순서상 뒤인 `fragment`의 값만 남는다 |
| `list`는 통째로 바뀐다 | 같은 키에 `list`를 담은 두 `fragment`, 읽기 권한 있음 | 조회 | 병합 순서상 뒤인 `fragment`의 `list`만 남는다. 이어 붙지 않는다 |
| 명시적으로 비운 값도 덮어쓴다 | 앞 `fragment`는 키에 값이 있고 뒤 `fragment`는 같은 키가 비어 있음, 읽기 권한 있음 | 조회 | 그 키는 비어 있다 |
...
`list`와 빈 값의 시나리오가 따로 있는 이유는, 이 병합이 `dict` 안으로만 재귀하고 나머지는
통째로 바꾸기 때문이다. `list`를 이어 붙이거나 빈 값을 건너뛰는 병합과 다르다.

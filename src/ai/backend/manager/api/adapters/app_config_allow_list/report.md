...
#### [a-forward-cursor-continues-after-the-named-row](/tests/scenario/bai_scenario/manager/app_config_allow_list/test_searching.py) — pass

항목 넷이 있고 슈퍼관리자가 기본 정렬의 첫 항목 다음 한 건을 커서로 검색하면, 다음 항목과 앞뒤 페이지가 모두 있다고 응답한다

Given

- 설정 이름 4개에 각각 공개 스코프 유형의 허용 목록 항목과 슈퍼관리자 한 명
  - 도메인 home-1
...
- AppConfigAllowListAdapter.admin_search — user-1이 wanted-1 항목의 커서 다음 한 건 검색

Then

- 커서 다음 항목과 앞뒤 페이지가 모두 있다고 응답한다
  - items[0].id: 커서 다음 ID와 같다
  - items = 1
  - total_count = 4
  - has_next_page = True
  - has_previous_page = True

#### [a-middle-offset-page-reports-both-directions](/tests/scenario/bai_scenario/manager/app_config_allow_list/test_searching.py) — pass

항목 넷을 설정 이름순으로 두 번째부터 두 건 검색하면, 중간 두 항목과 앞뒤 페이지가 모두 있다고 응답한다
...
- AppConfigAllowListAdapter.admin_search — user-1이 설정 이름순 결과의 두 번째 항목부터 두 건 검색
...
#### [an-or-filter-keeps-either-name](/tests/scenario/bai_scenario/manager/app_config_allow_list/test_searching.py) — pass

설정 이름 셋이 있고 슈퍼관리자가 두 이름을 OR로 묶어 검색하면, 두 이름 중 하나와 일치하는 항목만 반환된다

Given

- 설정 이름 3개에 각각 공개 스코프 유형의 허용 목록 항목과 슈퍼관리자 한 명
  - 도메인 home-1
  - 설정 정의 wanted-1: 이 이름의 설정이 등록돼 있다
  - 허용 목록 항목 entry-1: 공개 스코프에서 이 이름의 설정을 지정할 수 있다, 순위는 그 스코프 종류의 기본값
  - 설정 정의 other-1: 이 이름의 설정이 등록돼 있다
...
- AppConfigAllowListAdapter.admin_search — user-1이 wanted-1 또는 other-1인 항목 검색

Then

- 두 설정 이름 중 하나와 일치하는 항목만 반환된다
  - items = ['other-1', 'wanted-1']
  - total_count = 2
  - has_next_page = False
  - has_previous_page = False

#### [cursor-and-offset-pagination-may-not-be-mixed](/tests/scenario/bai_scenario/manager/app_config_allow_list/test_searching.py) — pass
...
- AppConfigAllowListAdapter.admin_search — user-1이 커서와 오프셋 페이지네이션을 함께 지정해 검색
...
설정 이름 둘에 항목 넷이 있고 슈퍼관리자가 이름 필터로 검색하면, 해당 이름의 항목만 반환된다
...
- AppConfigAllowListAdapter.admin_search — user-1이 설정 이름 wanted-1(으)로 검색
...
세 스코프 유형에 항목이 하나씩 있고 슈퍼관리자가 USER 유형으로 검색하면, USER 항목만 반환된다

Given

- 설정 이름 1개에 각각 공개·도메인·사용자 스코프 유형의 허용 목록 항목과 슈퍼관리자 한 명
  - 도메인 home-1
  - 설정 정의 wanted-1: 이 이름의 설정이 등록돼 있다
  - 허용 목록 항목 entry-1: 공개 스코프에서 이 이름의 설정을 지정할 수 있다, 순위는 그 스코프 종류의 기본값
...
- AppConfigAllowListAdapter.admin_search — user-1이 사용자 스코프 유형으로 검색

Then

- 사용자 스코프 유형의 항목만 반환된다
...
항목 11개가 있고 슈퍼관리자가 페이지 크기를 생략하면, 10건과 다음 페이지 표시가 반환된다

Given

- 설정 이름 11개에 각각 공개 스코프 유형의 허용 목록 항목과 슈퍼관리자 한 명
  - 도메인 home-1
  - 설정 정의 wanted-1: 이 이름의 설정이 등록돼 있다
  - 허용 목록 항목 entry-1: 공개 스코프에서 이 이름의 설정을 지정할 수 있다, 순위는 그 스코프 종류의 기본값
  - 설정 정의 other-1: 이 이름의 설정이 등록돼 있다
...
- AppConfigAllowListAdapter.admin_search — user-1이 순위 오름차순으로 검색

...
class VFolderAdapter(BaseAdapter):
    ...
    def _convert_vfolder_filter(self, f: VFolderFilter) -> list[QueryCondition]:
        ...
        if f.AND:
            conditions.extend(
                self.convert_and([self._convert_vfolder_filter(sub) for sub in f.AND])
            )
        if f.OR:
            conditions.extend(self.convert_or([self._convert_vfolder_filter(sub) for sub in f.OR]))
        if f.NOT:
            conditions.extend(
                self.convert_not([self._convert_vfolder_filter(sub) for sub in f.NOT])
            )
        return conditions

...
class SchedulingHistoryAdapter(BaseAdapter):
    ...
    async def admin_search_kernel_history(
        ...
        return AdminSearchKernelHistoriesPayload(
            items=[self._kernel_data_to_dto(h) for h in action_result.histories],
            total_count=action_result.total_count,
            has_next_page=action_result.has_next_page,
            has_previous_page=action_result.has_previous_page,
        )
    ...
    async def kernel_scoped_search(
        ...
        scope = KernelSchedulingHistorySearchScope(
            ...
            kernel_id=input.scope.kernel_id,
        )
        querier = self._build_kernel_querier(input)
        action_result = (
            await self._processors.scheduling_history.search_kernel_scoped_history.wait_for_complete(
                SearchKernelScopedHistoryAction(scope=scope, querier=querier)
            )
        )
        return AdminSearchKernelHistoriesPayload(
            items=[self._kernel_data_to_dto(h) for h in action_result.histories],
    ...
    def _convert_kernel_filter(self, filter: KernelHistoryFilter) -> list[QueryCondition]:
        ...
        if filter.phase is not None:
            ...
            if condition is not None:
                conditions.append(condition)
        if filter.from_status is not None and filter.from_status:
            conditions.append(KernelSchedulingHistoryConditions.by_from_statuses(filter.from_status))
        if filter.to_status is not None and filter.to_status:

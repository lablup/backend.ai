...
`login_client_type` 엔티티는 어느 스코프에도 속하지 않는다. 생성은 전역 역할이 있어야 하고,
하나를 지정해 수정·삭제하는 호출은 그 `login_client_type` 행에 부여된 권한을 검사하는데 그 권한을
...
| 시나리오 | 상황 | 요청 | 결과 |
|---|---|---|---|
| 슈퍼관리자가 이름만 지정해 생성한다 | `login_client_type` 없음, 전역 역할 있음 | 이름만 지정해 생성 | 설명이 비어 있는 노드 |
| 이름과 설명을 함께 지정해 생성한다 | `login_client_type` 없음, 전역 역할 있음 | 이름과 설명으로 생성 | 지정한 값이 그대로 담긴 노드 |
| 이미 사용 중인 이름으로 생성한다 | 그 이름의 `login_client_type` 있음, 전역 역할 있음 | 생성 | 이름 중복으로 거부 |
...
| 이름 필터로 검색한다 | 이름이 다른 `login_client_type` 여럿 | 이름 필터 조회 | 그 이름의 `login_client_type` 노드만 반환된다 |
| 페이지 크기를 생략한다 | `login_client_type` 51개 | 크기 없이 조회 | 50건까지 반환되고 다음 페이지가 있다고 응답한다 |
| `first`만 지정해 조회한다 | `login_client_type` 셋, 아무 권한도 없음 | `first` 값을 2로 지정해 조회 | 2건이 반환되고 다음 페이지가 있다고 응답한다 (실행에서 제외, 아래 참고) |

기본 페이지 크기는 다른 어댑터의 10이 아니라 50이다. "페이지 크기를 생략한다" 시나리오가 그 값을
검증한다.

`first`만 지정해 조회하는 시나리오는 현재 코드에서 실패하므로 실행에서 제외했다 (BA-7927).
요청 타입은 `first`·`after`·`last`·`before` 필드를 받지만 이 검색은 `limit` 값과 `offset` 값만
읽어서, `first`만 지정한 요청에 첫 페이지 전체를 반환한다. 시나리오에는 지정한 개수만 반환되는
결과를 기대값으로 적어 두었다. 고쳐지면 다시 실행에 포함한다.

## 수정

| 시나리오 | 상황 | 요청 | 결과 |
|---|---|---|---|
| 슈퍼관리자가 이름만 바꾼다 | 설명이 있는 `login_client_type` 하나, 전역 역할 있음 | 이름 수정 | 이름은 새 값, 설명은 그대로 |
| 설명을 지운다 | 설명이 있는 `login_client_type`, 전역 역할 있음 | 설명을 비우는 수정 | 설명이 없어진다 |
| 값을 하나도 지정하지 않는다 | `login_client_type` 하나, 전역 역할 있음 | 빈 수정 | 아무것도 바뀌지 않은 노드 |
| 이미 사용 중인 이름으로 바꾼다 | `login_client_type` 둘, 전역 역할 있음 | 한쪽 이름을 다른 쪽 이름으로 수정 | 이름 중복으로 거부 |
| 존재하지 않는 id를 수정한다 | 다른 `login_client_type` 행만 있음, 전역 역할 있음 | 이름 수정 | 대상을 찾을 수 없어 거부 |
...
| 시나리오 | 상황 | 요청 | 결과 |
|---|---|---|---|
| 슈퍼관리자가 삭제한다 | `login_client_type` 하나, 전역 역할 있음 | 삭제 | 삭제한 id를 담은 응답 |
| 존재하지 않는 id를 삭제한다 | 다른 `login_client_type` 행만 있음, 전역 역할 있음 | 삭제 | 대상을 찾을 수 없어 거부 |

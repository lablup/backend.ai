...
async def get_resource_slots(request: web.Request) -> web.Response:
    ...
    known_slots = await root_ctx.unified_config.shared_config_loader.get_resource_slots()
...
async def get_vfolder_types(request: web.Request) -> web.Response:
    ...
    root_ctx: RootContext = request.app["_root.context"]
    vfolder_types = await root_ctx.unified_config.shared_config_loader.get_vfolder_types()
...
async def app_ctx(app: web.Application) -> AsyncGenerator[None, None]:
    ...
    if root_ctx.pidx == 0:
        await root_ctx.unified_config.shared_config_loader.register_myself()
    yield
    if root_ctx.pidx == 0:
        await root_ctx.unified_config.shared_config_loader.deregister_myself()

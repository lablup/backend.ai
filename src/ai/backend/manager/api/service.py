...
from ai.backend.manager.services.model_service.actions.scale import ScaleServiceReplicasAction
from ai.backend.manager.services.model_service.actions.start_service import StartModelServiceAction
...
class NewServiceRequestModel(LegacyBaseRequestModel):
    ...
    def to_start_action(
        ...
        return StartModelServiceAction(
            service_name=self.service_name,
            replicas=self.replicas,
            image=self.image,
            runtime_variant=self.runtime_variant,
...
async def try_start(request: web.Request, params: NewServiceRequestModel) -> TryStartResponseModel:
    ...
    result = await root_ctx.processors.model_service.start_model_service.wait_for_complete(action)
...
async def generate_token(request: web.Request, params: TokenRequestModel) -> TokenResponseModel:
    ...
    return TokenResponseModel(token=result.token)

...
@check_api_params(
    t.Dict({
        tx.AliasedKey(["name", "service_name", "clientSessionToken"]) >> "service_name": t.Regexp(
            r"^(?=.{4,24}$)\w[\w.-]*\w$", re.ASCII
        ),
...
async def scale(request: web.Request, params: Any) -> web.Response:
    ...
    async with root_ctx.db.begin_session() as db_sess:
        ...
        await db_sess.execute(query)
        return web.json_response({
            "current_route_count": len(endpoint.routings),
            "target_count": params["to"],
        })

...
class CreateContainerRegistryQuota(graphene.Mutation):
    ...
    async def mutate(
        ...
        try:
            match scope_id:
                case ProjectScope():
                    await graph_ctx.processors.registry_quota._service.create_quota(
                        scope_id, int(quota)
                    )
                case _:
...
class UpdateContainerRegistryQuota(graphene.Mutation):
    ...
    async def mutate(
        ...
        try:
            match scope_id:
                case ProjectScope(_):
                    await graph_ctx.processors.registry_quota._service.update_quota(
                        scope_id, int(quota)
                    )
                case _:
                    raise NotImplementedError("Only project scope is supported for now.")
...
class DeleteContainerRegistryQuota(graphene.Mutation):
    ...
    async def mutate(
        ...
        try:
            match scope_id:
                case ProjectScope(_):
                    await graph_ctx.processors.registry_quota._service.delete_quota(scope_id)
                case _:
                    raise NotImplementedError("Only project scope is supported for now.")

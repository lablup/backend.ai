    ...
    def to_updater_spec(self) -> ImageUpdaterSpec:
        ...
        if self.resource_limits is not Undefined:
            ...
            for limit_option in self.resource_limits:
                limit_data = {}
                if limit_option.min is not Undefined and limit_option.min is not None and len(limit_option.min) > 0:
                    limit_data["min"] = limit_option.min
                if limit_option.max is not Undefined and limit_option.max is not None and len(limit_option.max) > 0:

...
async def create(request: web.Request, params: Any) -> web.Response:
    ...
    (
        group_id,
        domain,
    ) = await root_ctx.repositories.group_config.repository.resolve_group_id_and_domain(
        params["group"], params["domain"]
    )
...
async def list_or_get(request: web.Request, params: Any) -> web.Response:
    ...
    if not request["is_superadmin"]:
        if request["is_admin"]:
            ...
            is_member = await root_ctx.repositories.group_config.repository.check_user_in_group(
                request["user"]["uuid"], group_id
            )
            if not is_member:
                raise GenericForbidden("Users cannot access group dotfiles of other groups")

    if params["path"]:
        get_action = GetDotfileAction(group_id=group_id, path=params["path"])
    ...
    list_action = ListDotfilesAction(group_id=group_id)
...
async def delete(request: web.Request, params: Any) -> web.Response:
    ...
    action = DeleteDotfileAction(group_id=group_id, path=params["path"])

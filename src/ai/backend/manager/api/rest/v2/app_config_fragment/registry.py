...
def register_v2_app_config_fragment_routes(
    ...
    superadmin-only — non-admins read fragments through a scoped search, one route per
    scope the fragments may be written at.
    ...
    Layout:
        ...
        POST   /domains/search    fragments at one domain scope  (auth, RBAC)
        POST   /users/search      fragments at one user scope    (auth, RBAC)
    ...
    registry.add(
        "POST", "/domains/search", handler.domain_scoped_search, middlewares=[auth_required]
    )
    registry.add("POST", "/users/search", handler.user_scoped_search, middlewares=[auth_required])

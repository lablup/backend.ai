...
def register_v2_app_config_fragment_routes(
    ...
    superadmin-only — non-admins read fragments through ``/scoped/search``, which is
    RBAC-validated against the domain / user scope named in the request body.
    ...
    Layout:
        ...
        POST   /scoped/search     fragments at one domain/user   (auth, RBAC)

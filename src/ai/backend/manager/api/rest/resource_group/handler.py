...
class ResourceGroupHandler:
    ...
    def __init__(self, *, resource_group: ResourceGroupProcessors) -> None:
    ...
    async def list_available_sgroups(
        ...
        result = await self._resource_group.list_allowed_sgroups.run(action)
        resp = ListScalingGroupsResponse(
            scaling_groups=[ScalingGroupItem(name=name) for name in result.resource_group_names],
        )

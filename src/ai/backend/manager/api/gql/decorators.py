...
def gql_root_field(
    ...
    *,
    ...
    To expose a field on the unauthenticated public endpoint, pass the ``@public`` marker via
    ``directives=[Public()]`` (see ``api.gql.directives.Public``); only do so for resolvers that
    are safe to run without an authenticated user.
    """
    return strawberry.field(
        description=_build_description(meta),
        name=name,

...
@strawberry.federation.type(name="DomainNode", keys=["id"], extend=True)
class Domain:
    id: ID = strawberry.federation.field(external=True)

    @classmethod
    def resolve_reference(cls, id: ID) -> "Domain":
        return cls(id=id)


@strawberry.federation.type(name="GroupNode", keys=["id"], extend=True)
class Project:
    id: ID = strawberry.federation.field(external=True)

    @classmethod
    def resolve_reference(cls, id: ID) -> "Project":
        return cls(id=id)

...
from ai.backend.manager.repositories.agent.query import QueryConditions as AgentQueryConditions
...
async def load_agents_by_ids(
    ...
    querier = BatchQuerier(
        pagination=OffsetPagination(limit=len(agent_ids)),
        conditions=[AgentQueryConditions.by_ids(agent_ids)],
    )
    ...
    agent_map = {detail.agent.id: detail for detail in action_result.agents}
    return [agent_map.get(AgentId(agent_id)) for agent_id in agent_ids]

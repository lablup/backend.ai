...
@strawberry.field(
    description="Added in 25.18.0. List scaling groups associated with a specific project"
)
async def scaling_groups(
    ...
    querier.conditions.append(ScalingGroupConditions.by_project(project))
...
@strawberry.field(
    description="Added in 25.18.0. List all scaling groups without project filtering (for administrators)"
)
async def all_scaling_groups(
    info: Info[StrawberryGQLContext],
    filter: Optional[ScalingGroupFilter] = None,
    order_by: Optional[list[ScalingGroupOrderBy]] = None,
    before: Optional[str] = None,

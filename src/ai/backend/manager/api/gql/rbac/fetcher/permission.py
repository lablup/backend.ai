...
async def fetch_permissions(
    ...
    action_result = await processors.permission_controller.search_permissions.wait_for_complete(
        SearchPermissionsAction(querier=querier)
    )

    nodes = [PermissionGQL.from_dataclass(data) for data in action_result.items]
    edges = [PermissionEdge(node=node, cursor=encode_cursor(str(node.id))) for node in nodes]

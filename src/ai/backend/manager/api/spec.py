...
async def render_graphiql_html(request: web.Request) -> web.Response:
    ...
    if not root_ctx.unified_config.config.api.allow_graphql_schema_introspection:
...
async def render_openapi_html(request: web.Request) -> web.Response:
    root_ctx: RootContext = request.app["_root.context"]
    if not root_ctx.unified_config.config.api.allow_openapi_schema_introspection:
        raise GenericForbidden
...
async def init(app: web.Application) -> None:
    ...
    if root_ctx.unified_config.config.api.allow_openapi_schema_introspection:
        log.warning(

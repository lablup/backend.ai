...
async def stream_pty(defer, request: web.Request) -> web.StreamResponse:
    ...
    ws = web.WebSocketResponse(max_msg_size=root_ctx.unified_config.config.manager.max_wsmsg_size)
...
async def stream_execute(defer, request: web.Request) -> web.StreamResponse:
    ...
    config = root_ctx.unified_config.config
...
async def stream_proxy(
    ...
    try:
        ...
        ws = web.WebSocketResponse(
            autoping=False,
            max_msg_size=root_ctx.unified_config.config.manager.max_wsmsg_size,
        )
        await ws.prepare(request)
        proxy = proxy_cls(
            ws,

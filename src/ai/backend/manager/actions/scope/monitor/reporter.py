...
class ScopeActionReporterMonitor(ScopeActionMonitor):
    ...
    A reporter message is a per-run notification carrying a single ``entity_id``,
    so a scope run reports once with ``entity_id=None`` rather than fanning out per
    target scope; the per-scope trail is the audit-log monitor's job.
    """

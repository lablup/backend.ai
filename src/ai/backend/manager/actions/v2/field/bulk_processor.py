...
class BulkFieldActionProcessor[TAction: BaseBulkFieldAction[Any, Any], TData]:
    ...
    async def run(self, action: TAction) -> BulkFieldOpsResult[TData]:
        ...
        try:
            ...
            try:
                ...
                entity_results = self._per_row_results(result, owners)
                return result
        finally:
    ...
    def _per_row_results(
        self,
        result: BulkFieldOpsResult[TData],
        owners: Mapping[FieldIdentifier, EntityIdentifier],
    ) -> Sequence[BulkEntityResult]:
        """One record per named row, each naming the entity that answered for it.

        Which row it was goes in the description: the entity columns take entity ids
        only, and rows of one owner would otherwise be indistinguishable.
        """

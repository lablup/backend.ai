...
class AgentRegistry:
    ...
    async def _sync_agent_resource_and_get_kerenels(
        ...
        return SimpleKernelRegistry.from_json(resp)
    ...
    async def sync_agent_resource(
    ...
    ) -> dict[AgentId, SimpleKernelRegistry | MultiAgentError]:
        result: dict[AgentId, SimpleKernelRegistry | MultiAgentError] = {}
        ...
        for aid, task in aid_task_list:
            ...
            if agent_errors:
                ...
                assert isinstance(
                    resp, SimpleKernelRegistry
                ), f"response should be `SimpleKernelRegistry`, not {type(resp)}"
                result[aid] = resp
        return result

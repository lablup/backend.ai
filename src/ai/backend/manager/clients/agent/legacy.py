"""
Legacy agent client for backward compatibility.

This module provides the old AgentClient interface that creates
a new connection per RPC call. Use AgentClientPool for connection pooling.
"""
...
legacy_agent_client_resilience = Resilience(
    policies=[
...
class LegacyAgentClient:
    """
    Legacy client for communicating with a single agent via RPC.

    This class creates a new connection per RPC call.
    For connection pooling, use AgentClientPool instead.

    Deprecated: Use AgentClientPool.acquire() for new code.
    """

    _agent_cache: AgentRPCCache
    _agent_id: AgentId
    _invoke_timeout: float | None
    _order_key: str | None
    ...
    @legacy_agent_client_resilience.apply()

...
def populate(cli_ctx: CLIContext, fixture_path: Path) -> None:
    async def _impl():
        ...
        try:
            ...
            return
        db_username = cli_ctx.bootstrap_config.db.user
        db_password = cli_ctx.bootstrap_config.db.password
        db_addr = cli_ctx.bootstrap_config.db.addr.to_legacy()
        db_name = cli_ctx.bootstrap_config.db.name

...
def config_alembic(
    ...
    logger_root_level = ask_string(
        "Logger root level", default=config_parser["logger_root"]["level"]
    )
    logger_root_handlers = ask_string(
        "Logger root handlers", default=config_parser["logger_root"]["handlers"]
    )
    logger_root_qualname = ask_string(
        "Logger root qualname", default=config_parser["logger_root"]["qualname"]
    )
    ...
    logger_sqlalchemy_level = ask_string(
        "Logger sqlalchemy level", default=config_parser["logger_sqlalchemy"]["level"]
    )
    logger_sqlalchemy_handlers = ask_string(
        "Logger sqlalchemy handlers", default=config_parser["logger_sqlalchemy"]["handlers"]
    )
    logger_sqlalchemy_qualname = ask_string(
        "Logger sqlalchemy qualname", default=config_parser["logger_sqlalchemy"]["qualname"]
    )
    ...
    logger_alembic_level = ask_string(
        "Logger alembic level", default=config_parser["logger_alembic"]["level"]
    )
    logger_alembic_handlers = ask_string(
        "Logger alembic handlers", default=config_parser["logger_alembic"]["handlers"]
    )
    logger_alembic_qualname = ask_string(
        "Logger alembic qualname", default=config_parser["logger_alembic"]["qualname"]
    )
    ...
    handler_console_class = ask_string(
        "Handler console class", default=config_parser["handler_console"]["class"]
    )
    handler_console_args = ask_string(
        "Handler console args", default=config_parser["handler_console"]["args"]
    )
    handler_console_level = ask_string(
        "Handler console level", default=config_parser["handler_console"]["level"]
    )
    handler_console_formatter = ask_string(
        "Handler console formatter", default=config_parser["handler_console"]["formatter"]
    )

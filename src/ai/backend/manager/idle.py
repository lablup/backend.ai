...
class IdleCheckerHost:
    ...
    async def start(self) -> None:
        self._frozen = True
        _raw_config = await self._shared_config.etcd.get_prefix_dict(
            "config/idle/checkers",
        )
        raw_config = cast(Mapping[str, Mapping[str, Any]], _raw_config)
    ...
    async def get_idle_check_report(
        ...
        async def add_period_to_result(
            checker: BaseIdleChecker, grace_period: Optional[float]
        ) -> Optional[float]:

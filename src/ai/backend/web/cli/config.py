"""
Configuration management CLI commands for Backend.AI Web Server.
...
@click.option(
    ...
    default="./configs/webserver/sample.toml",
...
def generate_sample(
    ...
    """
    Generate a sample configuration file from the WebServerUnifiedConfig schema.
    ...
    model definition of WebServerUnifiedConfig.
    """
...
@click.option(
    ...
    default="./configs/webserver/halfstack.yaml",
    help="Path to the configuration file to validate. (default: ./configs/webserver/halfstack.yaml)",
)
def validate(
    path: pathlib.Path,
) -> None:
    """
    Validate a configuration file using the WebServerUnifiedConfig schema.
    
    This command loads and validates a TOML configuration file against
    the WebServerUnifiedConfig Pydantic model to ensure it's valid.
    """
    try:
        with open(path, "rb") as f:
            config_data = tomli.load(f)
        
        config = WebServerUnifiedConfig.model_validate(config_data)
        config.model_dump()

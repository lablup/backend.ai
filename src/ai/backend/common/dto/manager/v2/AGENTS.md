...
- Callers must never send `SENTINEL` over the wire — `SentinelOmittingMixin` drops it during serialization,
  so an omitted field and an explicit null stay distinguishable.

...
class EntityShareRecipientInput(BaseRequestModel):
    """Who an offer goes to, named exactly one of three ways.
    ...
    def _exactly_one(self) -> EntityShareRecipientInput:
        named = [self.project_id is not None, self.user_id is not None, self.email is not None]
        if sum(named) != 1:

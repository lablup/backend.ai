...
class AuthorizeRequest(BaseRequestModel):
    ...
    stoken: Optional[str] = Field(default=None, description="Session token")

    @field_validator("stoken", mode="before")
    @classmethod
    def normalize_stoken(cls, v: Optional[str], info) -> Optional[str]:
        """Handle both 'stoken' and 'sToken' field names."""
        if v is not None:
            return v
        # Check if sToken was provided in the original data
        if hasattr(info, "data") and info.data:
            return info.data.get("sToken")
        return None

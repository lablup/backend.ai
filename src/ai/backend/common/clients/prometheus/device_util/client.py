...
from .data.request import DeviceUtilizationQueryParameter
from .data.response import DeviceUtilizationQueryResult


class DeviceUtilizationReader(PrometheusHTTPClient):
    def _get_label_values_for_query(self, label: DeviceUtilizationQueryParameter) -> list[str]:
        ...
        _append_if_not_none(label.device_metric_name, "device_metric_name")
    ...
    def _get_sum_by_for_query(self, label: DeviceUtilizationQueryParameter) -> list[str]:
    ...
    async def get_device_utilization(
        self, param: DeviceUtilizationQueryParameter
    ) -> list[DeviceUtilizationQueryResult]:
        label_values = self._get_label_values_for_query(param)
        spec = QueryStringSpec(param.device_metric_name, self._timewindow, [], label_values)
        query_string = f"backendai_device_utilization{spec.str_labels()}[{spec.timewindow}]"
        ...
        return_val: list[DeviceUtilizationQueryResult] = []
    ...
    async def get_device_utilization_gauge(
        self, param: DeviceUtilizationQueryParameter
    ) -> list[DeviceUtilizationQueryResult]:
        label_values = self._get_label_values_for_query(param)
        sum_by_values = self._get_sum_by_for_query(param)
        spec = QueryStringSpec(
            param.device_metric_name, self._timewindow, sum_by_values, label_values
        )
        query_string = f"{spec.str_sum_by()}(backendai_device_utilization{spec.str_labels()})"
        query_data = QueryData(
            query_string,
            start=param.range.start_iso if param.range else None,
            end=param.range.end_iso if param.range else None,
    ...
    async def get_device_utilization_rate(
        self, param: DeviceUtilizationQueryParameter, interval: float = UTILIZATION_METRIC_INTERVAL
    ) -> list[DeviceUtilizationQueryResult]:
        label_values = self._get_label_values_for_query(param)
        sum_by_values = self._get_sum_by_for_query(param)
        spec = QueryStringSpec(
        ...
        query_string = (
            f"{spec.str_sum_by()}(rate(backendai_device_utilization{spec.str_labels()}[{spec.timewindow}])) "
            f"/ {interval}"
        )
        query_data = QueryData(
            query_string,
    ...
    async def get_device_utilization_diff(
        self, param: DeviceUtilizationQueryParameter
    ) -> list[DeviceUtilizationQueryResult]:
        label_values = self._get_label_values_for_query(param)
        sum_by_values = self._get_sum_by_for_query(param)
        ...
        query_string = f"{spec.str_sum_by()}(rate(backendai_device_utilization{spec.str_labels()}[{spec.timewindow}]))"
        query_data = QueryData(
            query_string,
            start=param.range.start_iso if param.range else None,
            end=param.range.end_iso if param.range else None,

...
from etcetra.client_v2 import EtcdClient, EtcdTransactionAction
...
class AsyncEtcd:
    ...
    def _parse_range_end_from_key(self, key: str) -> bytes:
    ...
    async def put(
        ...
        *,
        ...
        mangled_key = self._mangle_key(f"{_slash(scope_prefix)}{key}").encode(self.encoding)
        async with self.etcd.connect() as communicator:
            await communicator.put(mangled_key, MsgpackSerializer.serialize(val))
        ...
        *,
        ...
        def _txn(action: EtcdTransactionAction):
            for k, v in flattened_dict.items():
                key = self._mangle_key(f"{_slash(scope_prefix)}{k}").encode(self.encoding)
                value = MsgpackSerializer.serialize(v)
        ...
        *,
        ...
        async with self.etcd.connect() as communicator:
            for scope_prefix in scope_prefixes:
                key = self._mangle_key(f"{_slash(scope_prefix)}{key}").encode(self.encoding)
                value = await communicator.get(key)
                if value is not None:
                    return MsgpackSerializer.deserialize(value)
        return None
        ...
        *,
        ...
        async with self.etcd.connect() as communicator:
            for scope_prefix in scope_prefixes:
                ...
                values = await communicator.get_range(
                    mangled_key_prefix.encode(self.encoding),
                    self._parse_range_end_from_key(mangled_key_prefix),
                )
    ...
    async def delete_prefix(
        ...
        *,
        ...
        mangled_key_prefix = self._mangle_key(f"{_slash(scope_prefix)}{key_prefix}")
        async with self.etcd.connect() as communicator:
            await communicator.delete_range(
                mangled_key_prefix.encode(self.encoding),
                self._parse_range_end_from_key(mangled_key_prefix),
            )
    ...
    async def watch_prefix(
        ...
        *,
        ...
        while not ended_without_error:
            try:
                async for ev in self._watch_impl(
                    lambda communicator: communicator.watch_range(
                        mangled_key_prefix.encode(self.encoding),
                        self._parse_range_end_from_key(mangled_key_prefix),
                        ready_event=ready_event,
                    ),

"""singularity (apptainer) agent backend (BEP-1062, rootless).

Selectable via ``agent.backend = "singularity"``. Runs the same Linux session containers as the
containerd backend, but launches them through the **apptainer** CLI (sandbox image + overlay + userns)
...
implementation for :class:`~ai.backend.agent.singularity.runtime.SingularityRuntime`. The single seam is
...
from .agent import SingularityAgent, create_runtime


class SingularityAgentDiscovery(AbstractAgentDiscovery):
    @override
...
def get_agent_discovery() -> AbstractAgentDiscovery:
    return SingularityAgentDiscovery()

...
class ResourceAllocationConfig(BaseConfigSchema):
    ...
    @classmethod
    def _validate_devices_format(cls, v: Any) -> Mapping[DeviceName, Sequence[DeviceId]]:
        if v is None:
            return {}
        ...
        for key, value in v.items():
            device_name = DeviceName(str(key))
            ...
            case ResourceAllocationMode.MANUAL:
                for config in agent_configs:
                    if config.resource.allocations is None:
                        raise ValueError(
                            "On MANUAL mode, config must specify allocations (cpu, mem, devices)"
                        )
                    alloc = config.resource.allocations
                    if not alloc.cpu and not alloc.devices:
                        raise ValueError(
                            "On MANUAL mode, allocations must specify cpu or devices "
                            "(e.g., cpu = ['0', '1'], cuda = ['cuda0'])"
                        )

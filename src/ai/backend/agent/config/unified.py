...
class OverridableResourceConfig(BaseConfigSchema):
    ...
    allocated_disk: Optional[BinarySizeField] = Field(
        default=None,
        description=textwrap.dedent("""
        Hard disk allocation for this agent (e.g., "200G").
        Only used in MANUAL allocation mode.
        All agents must specify this value when allocation-mode is MANUAL.
        """),
        examples=["200G", "500G"],
        validation_alias=AliasChoices("allocated-disk", "allocated_disk"),
        serialization_alias="allocated-disk",
    )
    ...
    def validate_values_are_positive(self) -> Self:
        ...
        if self.allocated_disk is not None and self.allocated_disk < 0:
            raise ValueError(
                f"Allocated disk must not be a negative value, but given {self.allocated_disk}"
            )
    ...
    def _validate_resource_allocation_mode(self) -> Self:
        def validate_manual_resource_not_specified(config: AgentSpecificConfig) -> None:
            ...
            if any([
                ...
                resource.allocated_disk is not None,
        ...
        def validate_mandatory_manual_resource_specified(config: AgentSpecificConfig) -> None:
            ...
            if any([
                ...
                resource.allocated_disk is None,
            ]):
                raise ValueError(
                    "On MANUAL mode, config must specify cpu, mem, and disk resource allocations"
                )

...
@click.option(
    ...
    default="./configs/agent/halfstack.yaml",
    help="Path to the configuration file to validate. (default: ./configs/agent/halfstack.yaml)",
)
def validate(
    path: pathlib.Path,
) -> None:
    """
    Validate a configuration file using the AgentUnifiedConfig schema.
    
    This command loads and validates a TOML configuration file against
    the AgentUnifiedConfig Pydantic model to ensure it's valid.
    """
    try:
        with open(path, "rb") as f:
            config_data = tomli.load(f)
        
        config = AgentUnifiedConfig.model_validate(config_data)
        config.model_dump()

"""Intrinsic compute plugins (CPU and main memory) — backend-agnostic.

These are the same plugins for every container runtime: they read the cgroup hierarchy, which
belongs to the kernel, not to Docker. They used to live in ``agent/docker/intrinsic.py`` and
construct an aiodocker client in ``init()``, which meant a containerd-only node — with no Docker
installed and no ``/var/run/docker.sock`` — could not even start its agent.

Nothing here talks to a container runtime. The one sample that genuinely comes from a runtime API
(``StatModes.DOCKER``) is fetched through ``AbstractAgent.fetch_container_api_stats``, which the
Docker backend implements and every other backend leaves at its default. Everything else goes
through the runtime-neutral agent hooks: ``get_cgroup_path``, ``get_cgroup_version``,
``get_container_netns`` and ``enumerate_container_pids``.
"""
    ...
    async def gather_container_measures(
        ...
        async def sysfs_impl(
            ...
            try:
                ...
                return None
            container_pid = (netns.pid if netns else None) or _INVALID_PID
            ns_path = netns.path if netns else None
            ...
            elif ns_path and ns_path.exists():
                try:

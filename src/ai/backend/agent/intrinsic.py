"""The intrinsic compute plugins (CPU and main memory), for every container runtime.

They read the cgroup hierarchy, which belongs to the kernel and not to any runtime, so one module
serves the Docker and the containerd backends alike. What genuinely differs per runtime is asked of
the agent through three hooks -- `get_container_netns`, `fetch_container_api_stats` and
`container_storage_root` -- and a backend without an answer leaves them at their default.
"""
        ...
        async def api_impl(container_id: str) -> float | None:
            try:
                async with asyncio.timeout(_CONTAINER_STAT_TIMEOUT):
                    ret = await ctx.agent.fetch_container_api_stats(container_id)
            except TimeoutError:
                return None
            if ret is None:
                return None
...
class MemoryPlugin(AbstractComputePlugin):
    ...
    #: The runtime's storage root with a trailing slash, learned from the agent at the first node
    #: sample; None until then, and stays None on a runtime that names no root.
    ...
    async def _get_graph_root_prefix(self, ctx: StatContext) -> str | None:
        graph_root = await ctx.agent.container_storage_root()
        if not graph_root:
            return None
    ...
    async def gather_node_measures(self, ctx: StatContext) -> Sequence[NodeMeasurement]:
        ...
        if self._graph_root_prefix is None:
            self._graph_root_prefix = await self._get_graph_root_prefix(ctx)
    ...
    async def gather_container_measures(
        ...
        async def sysfs_impl(
            ...
            try:
                async with asyncio.timeout(_CONTAINER_STAT_TIMEOUT):
                    netns = await ctx.agent.get_container_netns(container_id)
            except TimeoutError:
                log.warning(
                ...
                return None
            container_pid = (netns.pid if netns is not None else None) or _INVALID_PID
            ns_path = netns.path if netns is not None else None
            ...
            if container_pid > 0:
                ...
                log.warning(
                    "MemoryPlugin: container {0} has no PID and no pinned netns,"
    ...
    async def restore_from_container(
        ...
        # The limit the runtime holds when it reports one (Docker's inspect); otherwise the
        # allocation BAI itself wrote into the container's config directory.
        memory_limit: Any = nmget(container.backend_obj, "HostConfig.Memory")

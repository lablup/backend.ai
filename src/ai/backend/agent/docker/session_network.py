"""Session networking (BEP-1079) for the Docker backend.
...
BEP-1079 backend gets ``NetworkMode: none`` and is attached by PID, instead of being handed to
...
def is_session_networked(cluster_info: ClusterInfo) -> bool:
    ...
    manager's network config names a BEP-1079 data plane (vxlan, or a node-local bridge). Its
...
def effective_privnet_socket(configured: str | None, cluster_driver: str | None) -> str | None:
    ...
    A cluster whose multi-node sessions ride Docker Swarm (``overlay``) has no use for the BEP-1079

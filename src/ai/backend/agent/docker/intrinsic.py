...
The intrinsic CPU/memory compute plugins live in ``ai.backend.agent.intrinsic``: they read cgroups,
which is the same on every runtime, and holding an aiodocker client here meant a containerd-only
node could not start its agent. They stay importable from this module.
"""

...
@web.middleware
async def container_resolver_middleware(request: web.Request, handler: Handler) -> web.StreamResponse:
...
class MetadataServer(aobject):
    ...
    @web.middleware
    async def route_structure_fallback_middleware(self, request: web.Request, handler: Handler) \
            -> web.StreamResponse:
        try:
            return await handler(request)

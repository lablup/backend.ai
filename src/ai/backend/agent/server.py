...
class AgentRPCServer(aobject):
    ...
    async def __ainit__(self) -> None:
        ...
        # Each backend contributes its own liveness probes (its container-runtime daemon), so
        # the server never branches on the backend here.
...
async def etcd_ctx(local_config: AgentUnifiedConfig) -> AsyncGenerator[AsyncEtcd]:
    ...
    scope_prefix_map = {
        ConfigScopes.GLOBAL: "",
        ConfigScopes.SGROUP: f"sgroup/{local_config.agent.legacy_sgroup_name()}",

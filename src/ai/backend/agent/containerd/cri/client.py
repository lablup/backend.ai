...
class CriClient:
    ...
    async def connect(self) -> None:
        ...
        # mypy-protobuf emits *AsyncStub names ONLY in the .pyi for type
        # checking — they don't exist at runtime, only the sync Stub
        # class is generated by grpc_tools.protoc. The sync class works
        # transparently against an aio channel (its method callables are
        # awaitable when the channel is aio), so we instantiate the real
        # sync class and cast it to the aio-typed alias for the type
        # checker.
        self._runtime_stub = cast(
            api_pb2_grpc.RuntimeServiceAsyncStub,
            api_pb2_grpc.RuntimeServiceStub(channel),
        )
        self._image_stub = cast(
            api_pb2_grpc.ImageServiceAsyncStub,

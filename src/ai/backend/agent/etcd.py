...
class AgentEtcdClientView(AbstractKVStore):
    ...
    def _agent_scope_prefix_map(self) -> Mapping[ConfigScopes, str]:
        ...
        """
        return {
            ConfigScopes.SGROUP: f"sgroup/{self._config.agent.legacy_sgroup_name()}",
    ...
    def _augment_scope_prefix_map(
        ...
        return ChainMap(
            cast(MutableMapping[ConfigScopes, str], override),
            cast(MutableMapping[ConfigScopes, str], self._agent_scope_prefix_map),
        )

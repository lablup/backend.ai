...
A pure (I/O-free) data structure the SessionNetwork owns: it records which
...
class SessionContainerTracker:
    ...
    def attach(self, session_id: str, kernel_id: str, container_id: str) -> None:
        """Track a container whose id is NOT its kernel id, retiring that kernel's claim.

        `track` alone is enough where the two coincide, which is the containerd assumption. Docker
        names its containers itself, so the claim `reserve` made under the kernel id and the entry
        `track` makes under the container id are different keys: the claim would outlive every
        container of the session and `untrack` would never report a last one, leaving the session's
        devices, its LOCAL block and its etcd membership pinned for the life of the agent.
        """
        self.track(session_id, container_id)
        if kernel_id == container_id:
            return
        # After `track`, so the session can never look empty in between.
        self._kernel_session.pop(kernel_id, None)
        if (pending := self._session_pending.get(session_id)) is not None:
            pending.discard(kernel_id)
            if not pending:
                self._session_pending.pop(session_id, None)

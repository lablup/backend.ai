"""What the privileged helper must remember across its own restart (BEP-1062).

The agent recovers by reading durable ground truth: containerd's labels say which container belongs
to which session, and etcd holds each session's meta and each endpoint's overlay IP. The helper can
...
agent that could re-declare a session's subnet on the helper's behalf could point a session's data
plane anywhere.

So the helper journals what it derived *itself*, at the moment it derived it, and on boot replays
that against containerd (which it does trust: it already re-resolves every container's PID there).
Without this, a helper restart leaves the node's devices up but the daemon that owns them empty:
...
- ``sessions/<session_id>`` — the validated network config the helper set the session up from. It
  is what lets a restarted helper rebuild the session's meta and re-derive its device names.
- ``attachments/<container_id>`` — the session it belongs to, and the overlay IP the helper
...
DEFAULT_HELPER_STATE_DIR = Path("/var/lib/backend.ai/net-helper")
...
class AttachRecord:
    """One container's attachment, as the helper itself resolved it."""
...
class HelperJournal:
    """The helper's own durable record of the sessions and attachments it owns."""

    _dir: Path

    def __init__(self, state_dir: Path | None = None) -> None:
        self._dir = state_dir if state_dir is not None else DEFAULT_HELPER_STATE_DIR
    ...
    def _read_all(self, kind: str) -> dict[str, dict[str, Any]]:
        ...
        for entry in sorted(directory.iterdir()):
            ...
            try:
                ...
                log.warning("dropping unreadable helper journal record {}", entry)
                continue
    ...
    async def sessions(self) -> dict[str, dict[str, Any]]:
        """``{session_id: network_config}`` for every session this helper set up and has not
        torn down."""
    ...
    async def attachments(self) -> dict[str, AttachRecord]:
        """``{container_id: AttachRecord}`` for every container this helper attached and has not
        detached."""
        ...
        for container_id, payload in (
            ...
            if not isinstance(session_id, str):
                log.warning("dropping helper attach record without a session: {}", container_id)
                continue

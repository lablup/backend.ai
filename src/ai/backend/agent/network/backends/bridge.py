...
class BridgeNetworkPlugin(AbstractNetworkAgentPluginV2[AbstractKernel]):
    ...
    def _index(self, session_id: str) -> int:
        """Assign a node-local /24 index for the session's bridge (idempotent)."""
        if (existing := self._indices.get(session_id)) is not None:
            return existing
        used = set(self._indices.values())
        for i in range(256):
            if i not in used:
                self._indices[session_id] = i
                return i
        raise RuntimeError("node-local bridge subnet pool exhausted (>256 sessions/node)")

    def _subnet(self, session_id: str) -> str:
        return f"{self._local_pool_prefix}.{self._index(session_id)}.0/24"

    def _bridge(self, session_id: str) -> str:
        return local_bridge_dev(self._index(session_id))
    ...
    async def setup_session_network(self, meta: SessionNetMeta, self_member: Member) -> None:
        ...
        await self._delete_link_quiet(self._bridge(meta.session_id))

    @override
    async def teardown_session_network(self, session_id: str) -> None:
        if session_id not in self._indices:
            return
        await self._delete_link_quiet(self._bridge(session_id))
            ...
            attachments=[
                NetworkAttachSpec(
                    ...
                    cni_config=local_cni_config(
                        meta.session_id,
                        bridge=self._bridge(meta.session_id),
                        subnet=self._subnet(meta.session_id),
                    ),
                ),
            ]
        )

...
class PrivNetOp(enum.StrEnum):
    ...
    # Re-assert the session's security state (the plaintext drop, the ESP pairs). Driven by the
    # agent's reconcile the same way the peer verbs are, but on a timer rather than a diff: what
    # this re-asserts can be removed by anything that touches the host and never changes a
    # published record.
...
class PrivNetRequest:
    ...
    ip: str | None = None
    mac: str | None = None
    # ENSURE_SECURITY only: every peer VTEP the session's published membership currently names.
    # Sent on every pass rather than diffed, because the point of this verb is to be independent of
    # what either side remembers -- a restarted privnet has no record of the pairs it protects, and
    # the agent will not resend an add_peer for a member whose record has not changed.
    vteps: tuple[str, ...] | None = None
    ...
    def encode(self) -> bytes:
        ...
        if self.vteps is not None:
            payload["vteps"] = list(self.vteps)
    ...
    def decode(cls, line: bytes) -> PrivNetRequest:
        ...
        for key in ("vtep_ip", "ip", "mac", "local_ip"):
            ...
            fields[key] = value
        raw_vteps = data.get("vteps")
        if raw_vteps is not None and not (
            isinstance(raw_vteps, list) and all(isinstance(v, str) for v in raw_vteps)
        ):
            raise ProtocolError("vteps must be a list of strings")
        vteps = tuple(raw_vteps) if raw_vteps is not None else None

"""Input policy for the privnet daemon (BEP-1079).
...
from ai.backend.common.network.types import DEFAULT_VXLAN_PORT, NetworkBackendKind
...
class ValidatedNetworkConfig:
    ...
    vni: int | None
    mtu: int
    #: The UDP port the tunnel runs on. Carried through because it is not cosmetic: it is what an
    #: operator moves the overlay off 4789 with when the fabric blocks that port, and it is part of
    #: the ESP policy's selector -- a privnet that defaulted it while the agent believed otherwise
...
def validate_network_config(raw: dict[str, Any]) -> ValidatedNetworkConfig:
    ...
    if port_raw is not None:
        vxlan_port = _as_int(port_raw, "vxlan_port")

"""Agent networking capability probe (BEP-1079).
...
def _owned_by(agent_id: str, boot_id: str | None) -> dict[str, str | None]:
    """The condition under which this run may change what its agent id advertises.

    An agent id restarted quickly has two processes alive at once. Reading the boot key and then
    writing is two operations, and between them the new run can publish its own advert -- which
    the old run's last refresh then lands on top of, or its shutdown deletes. Either way a node
    that is up and serving goes missing from every placement, or one that is gone stays
    advertised. So the condition travels WITH each write, in the same store operation, and a
    write that has lost the id simply does not land.
    """
    return {agent_boot_key(agent_id): boot_id} if boot_id is not None else {}


async def _own_put(
    etcd: AbstractKVStore, key: str, value: str, owned: Mapping[str, str | None]
) -> None:
    """Write ``key``, but only while this run still owns the agent id. See `_owned_by`."""
    if not owned:
        await etcd.put(key, value, scope=ConfigScopes.GLOBAL)
        return
    standing = await etcd.get(key, scope=ConfigScopes.GLOBAL)
    if not await etcd.compare_and_put(
        key, value, expected=standing, guards=dict(owned), scope=ConfigScopes.GLOBAL
    ):
        log.info("not writing {}: this run no longer owns this agent id, or it changed", key)


async def _own_delete(etcd: AbstractKVStore, key: str, owned: Mapping[str, str | None]) -> None:
    """Remove ``key``, but only while this run still owns the agent id. See `_owned_by`."""
    standing = await etcd.get(key, scope=ConfigScopes.GLOBAL)
    if standing is None:
        return
    if not owned:
        await etcd.delete(key, scope=ConfigScopes.GLOBAL)
        return
    if not await etcd.compare_and_delete(
        key, standing, guards=dict(owned), scope=ConfigScopes.GLOBAL
    ):
        log.info("not removing {}: this run no longer owns this agent id, or it changed", key)
...
async def publish_caps(
    ...
    *,
    ...
    owned = _owned_by(agent_id, boot_id)
    ...
    if standing == digest:
        ...
        await _own_put(etcd, agent_caps_key(agent_id), record, owned)
        return
    # Readiness has moved. These are separate writes and a manager can read between them, so the
    # order decides what it can see. Taking the OLD fence down first means the worst a reader gets
    # is "this node is not currently admitting" -- true, briefly. Writing the record first instead
    ...
    # changed.
    await _own_delete(etcd, agent_ready_key(agent_id), owned)
    await _own_put(etcd, agent_caps_key(agent_id), record, owned)
    await _own_put(etcd, agent_ready_key(agent_id), digest, owned)
...
async def withdraw_caps(etcd: AbstractKVStore, agent_id: str, boot_id: str | None = None) -> None:
    ...
    await _own_delete(etcd, agent_ready_key(agent_id), owned)
    await _own_delete(etcd, agent_caps_key(agent_id), owned)


async def publish_vtep(
    etcd: AbstractKVStore, agent_id: str, vtep_ip: str, boot_id: str | None = None
) -> None:
    ...
    Owned by the run that writes it, like the capability record: peers program their FDB straight
    from this, so an address an earlier run left here points them at a host that may no longer
    hold it."""
    await _own_put(etcd, agent_vtep_key(agent_id), vtep_ip, _owned_by(agent_id, boot_id))


async def withdraw_vtep(etcd: AbstractKVStore, agent_id: str, boot_id: str | None = None) -> None:
    ...
    Owned by the run that retracts it: a shutting-down process must not take away the address a
    newer run of the same agent id has just published."""
    await _own_delete(etcd, agent_vtep_key(agent_id), _owned_by(agent_id, boot_id))

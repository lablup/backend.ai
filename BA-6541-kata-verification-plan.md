...
| # | Item | BA mechanism | How to test | Pass criteria |
|---|------|-------------|-------------|---------------|
| 1 | Code execution | kernel-runner execute | Run a code snippet via session | Correct output returned |
| 2 | exec / attach (pty) | docker exec-based terminal | `./bai` exec / open terminal | Interactive shell works |
| 3 | Session lifecycle | start / restart / terminate + cleanup | restart, then terminate the session | Clean restart; terminate removes Kata VM and scratch |
...
| # | Item | BA mechanism | How to test | Pass criteria |
|---|------|-------------|-------------|---------------|
| 1 | CPU / Memory limits | `Cpus`, `CpusetCpus`, `Memory`, `MemorySwap` (`docker/intrinsic.py`) | Create with CPU/mem caps; check inside kernel | Limits enforced or documented approximation |
| 2 | GPU passthrough | accelerator plugin injects `Devices`/env | Create with GPU resource slot | GPU visible in kernel (likely needs VFIO setup ⇒ ⚠️/❌) |
| 3 | RDMA / hugepage | `/dev/infiniband` device, `IPC_LOCK`, `memlock` ulimit (`agent.py:717,1201`) | Create on RDMA-capable host | Device present & usable |
| 4 | shmem (`/dev/shm`) | `ShmSize` (`agent.py:1244`) | Create with `shmem` resource_opt; check `/dev/shm` | tmpfs sized correctly inside VM |
| 5 | vfolder / NFS-backed mount | vfolder host bind: `Path(host_path) → kernel_path` (`agent.py:559-565`) — same bind mechanism as scratch (Tier 1 #2) | Mount an NFS-backed vfolder; inside kernel `ls`, read/write a file, `mount` to confirm the path | Vfolder visible & R/W inside VM; an NFS-backed host path passes through the bind → virtio-fs hop with byte-exact data (note any locking/attr-cache caveats) |

> If the test host has no GPU/RDMA, mark GPU passthrough / RDMA (#2–#3) **Not
> tested** with the reason, rather than guessing.
...
| # | Item | BA mechanism | How to test | Pass criteria |
|---|------|-------------|-------------|---------------|
| 1 | Port binding / service port | `PortBindings` host↔container (`agent.py:1196`) | Inspect published ports; reach a service port | Port mapping works through VM network |
| 2 | App Proxy access | wsproxy/appproxy → kernel service port | Open an app (e.g. Jupyter) via proxy | App reachable end-to-end (depends on #1) |
| 3 | Overlay networking | multi-node cluster network | Multi-node session (if available) | Inter-node connectivity (single-node ⇒ defer) |
...
| # | Item | BA mechanism | How to test | Pass criteria |
|---|------|-------------|-------------|---------------|
| 1 | JAIL sandbox mode | `seccomp=unconfined`, `apparmor=unconfined`, `SYS_PTRACE` (`agent.py:1227-1236`) | Set `container.sandbox_type = JAIL`, create kernel | Kernel starts & sandbox functions under Kata |
| 2 | seccomp profile | `_apply_seccomp_profile` (`agent.py:1053,1222`) | Default sandbox create | Profile applied without breaking the VM |
...
| # | Item | Result | Evidence | Notes (extra config / limitation) |
|---|------|--------|----------|-----------------------------------|
| 1 | Code execution | ✅ | Jupyter cell `print(1+1)` → `2`; `platform.uname().release` → `6.18.28` (Kata guest kernel, differs from host) | kernel-runner executes inside the Kata VM |
| 2 | exec / attach | ✅ | Jupyter Terminal (pty) interactive shell works | docker exec / pty allocation OK under Kata |
| 3 | Session lifecycle | ✅ | `terminate` → session TERMINATED; kernel container, Kata VM (hypervisor), virtiofsd, kata shim, and scratch dir all removed | Clean teardown — no leaked VM/virtiofsd processes |

**Tier 3 — Resources & devices**

| # | Item | Result | Evidence | Notes (extra config / limitation) |
|---|------|--------|----------|-----------------------------------|
| 1 | CPU / Memory limits | ✅ | Session allocated cpu=2 / mem=4 GiB. **CPU:** `nproc=2` inside the Kata guest (cpuset effective), identical to a runc baseline kernel with the same alloc; `/proc/cpuinfo` shows 3 (Kata VM vCPUs) vs 8 (runc → full host) — view differs, effective limit identical. **Memory:** `MemTotal` shows the VM size (~6 GB Kata / 32 GB runc) — not the cap; cgroup limits never appear in `/proc/meminfo` (normal Linux, true on runc too). Actual enforcement confirmed: committing 5 GiB > 4 GiB limit → OOM `Killed` on **both** Kata and runc | CPU pinning via `CpusetCpus`; `HostConfig.Cpus` is a no-op (not a real Docker field, silently dropped). Memory cap enforced at the host sandbox cgroup. Guest-view differences (cpuinfo/MemTotal) are runtime-topology artifacts, not limit failures — validated against a runc baseline |
| 2 | GPU passthrough | ➖ | No GPU on the Vultr bare-metal host | Not tested — requires GPU + VFIO setup under Kata |
| 3 | RDMA / hugepage | ➖ | No `/dev/infiniband` on the host | Not tested — requires RDMA-capable host |
| 4 | shmem | | | |
| 5 | vfolder / NFS-backed mount | ✅ | Session created with an NFS-backed vfolder mounted; the mount is visible in the kernel and read + write both succeed inside the Kata guest | NFS host_path bind → virtio-fs passthrough into the VM works with no extra config (pending: capture guest `mount` type = virtiofs and host `df -T` = nfs4 to finalize evidence) |

**Tier 4 — Networking & external access**

| # | Item | Result | Evidence | Notes (extra config / limitation) |
|---|------|--------|----------|-----------------------------------|
| 1 | Port binding | ✅ | jupyter service port reached end-to-end through proxy chain (coordinator:10200 → worker:10201 → app:10240) | Service port traverses Kata VM network with no extra config |
| 2 | App Proxy | ✅ | `start-service` → `/v2/proxy/auth` → worker `/setup` (cookie) → Jupyter Notebook 7.3.3 returned `200 OK` + full HTML | wsproxy/appproxy end-to-end works; `rootUri=file:///home/work` confirms VM scratch mount |
| 3 | Overlay networking | ✅ | `cluster_mode=multi-node, cluster_size=2` session placed kernels on two agents (ag1/ag2); Docker swarm overlay `bai-multinode-<id>` created with both nodes as VXLAN peers; kernels got overlay IPs `10.0.1.4` (main1) / `10.0.1.2` (sub1); cross-node TCP connect sub1 → `10.0.1.4:12345` succeeded (`OK: overlay reachable`) | Kata VMs participate in the swarm overlay; inter-node connectivity confirmed end-to-end. **Caveat:** swarm `advertise-addr` was the public IP, so VXLAN (UDP 4789) traverses the public interface in plaintext — for production re-init swarm on the private VPC IP. MTU left at 1500. |

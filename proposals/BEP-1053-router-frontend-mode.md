...
|---|---|---|
| Key generation / issuance | **User** | An explicit action via WebUI / API / CLI; the Manager mints the token and persists it with its owner. |
...
- **Router API keys.** Issuance is a user-initiated action (WebUI / API /
  CLI); the Manager mints the key material (an opaque `sk-…`-style token)
  and persists it with the owning user/project and expiry — the
  `EndpointTokenRow` pattern lifted from endpoint scope to model scope.
  Issuing, listing, rotating, and revoking are Manager operations.

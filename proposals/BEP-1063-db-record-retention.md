...
| `roles_invitations` | `roles`, `vfolder_invitations`, `entity_shares` | roles `deleted_at`+`DELETED`; `entity_shares` splits in two — an offer still waiting goes on `expires_at` having passed, an ended one on terminal state + `updated_at` | 1yr | domain group (invitations unified), per-table spec |

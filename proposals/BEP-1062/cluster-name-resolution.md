...
### Privileged-port binding: the dockerd redirect model, on an ephemeral loopback port (settled)

The resolver must answer on `gateway:53` (glibc always queries port 53), but binding a port `<1024` needs `CAP_NET_BIND_SERVICE`/root. Having the **agent** bind `:53` would force it to be privileged — defeating privnet's entire purpose (an unprivileged agent, privnet holding the caps). This is exactly the problem dockerd's embedded DNS solves (127.0.0.11 on a high port + an iptables `:53` redirect), and we adopt its shape:

- The resolver binds **`127.0.0.1:0`** — an **ephemeral loopback** port. Loopback ⇒ it sits on no real interface; port 0 ⇒ the OS assigns a guaranteed-free port that can never collide (unlike any fixed high port). The agent reads the assigned port back off the socket.
- The **one privileged step** — an iptables `PREROUTING -d gateway -p udp --dport 53 -j DNAT --to-destination 127.0.0.1:<port>` (plus `net.ipv4.conf.all.route_localnet=1`, since Linux drops martian packets routed to 127/8 by default — the same knob dockerd sets) — is installed by the privileged holder: **the privnet** (new RPC `SETUP_DNS_REDIRECT`, the agent sending only the loopback port; the privnet derives the gateway from the session it owns) or the **in-process backend** (direct). Either way the **agent process needs no network privilege and no fixed port**.
- The container's `/etc/resolv.conf` still points at `gateway:53` (unchanged); the redirect is transparent.

**Bind-then-redirect ordering + comment-tagged rules make the ephemeral port recovery-safe.** `ensure_cluster_dns` binds the resolver first, then installs the redirect to the port it got, so `:53` never points at a dead socket. The DNAT is tagged `bai-dns:<session_id>`. A distinct ephemeral port per session also means the loopback address is shared but each session's redirect is unambiguous, and the "two agents share one gateway `:53`" concern dissolves (per-session-gateway rule, single privileged owner).

**Stale-rule cleanup (found in workload validation).** Two bugs the live 2-node all-reduce surfaced, both now fixed and regression-tested:
- `iptables -S` prints the comment value **quoted** (`--comment "bai-dns:x"`); replaying that token verbatim as `-D` never matches, so the rule silently leaked — even on a clean teardown. The removal now strips the quotes.
- **Install clears every DNS rule on the gateway, not just this session's tag.** A session that dies *without* a clean teardown (e.g. an agent↔Valkey heartbeat timeout) leaks its rule; when the LOCAL block is later reused, that stale rule sits first in `PREROUTING` and DNATs `:53` to a now-dead loopback port, shadowing the new session's resolver. Matching by `-d <gateway>/32` (a gateway serves one session at a time) removes it on install. Teardown still removes only this session's tag.

Why loopback over the gateway (an earlier iteration): the gateway also works and needs no `route_localnet`, but loopback keeps the socket off every real interface and forces the per-session-ephemeral-port scheme that makes collisions structurally impossible — the safer end state.

**Live-validated 2-node:** the resolver bound `127.0.0.1:56163` (a random ephemeral port; **nothing on any interface's `:53`**), the `gateway:53 → 127.0.0.1:56163` DNAT was present and `route_localnet=1`, and a container's `getent hosts sub1` resolved cross-node through the redirect while non-cluster names forwarded to CoreDNS.

### Fail-loud (settled)

Because phase 5 removed the `/etc/hosts` peer fallback, a resolver that fails to start leaves cluster names unresolvable and the session hangs at rendezvous with no visible cause. So `ensure_cluster_dns` **fails the kernel** (`ClusterDNSStartError`) on a bind failure, a missing post-attach LOCAL block, or a failed redirect — rather than degrading silently. A genuinely absent session (no coordinator on this node) is still a no-op, and the **resume/recovery path is best-effort** (a resolver hiccup must not abort re-adopting already-running containers — the `recover()` contract).

...
4. **Daemon wiring — done (pending live validation).** `ContainerdSessionNetwork` starts one `ClusterDNSServer` per session on that session's LOCAL gateway (`local_gateway_of` = first host of the session's LOCAL subnet), with `SessionClusterNames(coordinator, session_id)` as the source and the container-DNS upstreams as the forwarder; it is torn down with the coordinator. The container's `/etc/resolv.conf` lists the gateway **first**, then the upstreams (`_prepare_resolv_conf`). Start is best-effort — no gateway or a failed bind is logged, not fatal. It runs **alongside the full static `/etc/hosts`** (nsswitch `files` before `dns`), so if the resolver is absent nothing regresses: cluster names still resolve via the file and non-cluster names via the upstream fallback in resolv.conf. Unit-tested (`test_session_network_dns.py`, `test_coordinator.py::…SessionClusterNames`). Live multi-node validation (redeploy fatpods; `dig @gateway sub1`) still pending before phase 5.
5. **Remove the static peer map from `/etc/hosts`** — the file keeps only `localhost` + self; peers resolve via the resolver. This deletes `_peer_host_map`'s peer-map construction and the per-backend hardening it required (the divergence collapses into the one resolver path). Gate on the phase-4 live validation. **Prerequisites to settle here:**
   - **Single-node name source** (blocking): single-node sessions have no `endpoints/` entries, so the resolver cannot name them — feed the agent's `cluster_host_ips` map into a `ClusterNameSource` for the bridge path, or keep `/etc/hosts` for single-node only.
   - **Node-level resolver ownership**: two agents on one node share one gateway, so only one can bind `:53` — make the resolver a node-level (not per-agent) singleton, or have the surviving agent re-own it on withdraw.

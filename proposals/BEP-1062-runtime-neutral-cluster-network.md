...
detail-docs: [control-plane.md, agent-plugin-v2.md, data-plane-backends.md, cluster-name-resolution.md, overlay-encryption.md, migration.md, verification.md]
key-constraints:
...
| [overlay-encryption](./BEP-1062/overlay-encryption.md) | Kernel IPSec (ESP/AES-GCM) on the VXLAN tunnel; session key via etcd meta; XFRM programmed beside the FDB |
...
| P10 | Cluster name resolution ([cluster-name-resolution](./BEP-1062/cluster-name-resolution.md)): static `/etc/hosts` hardening **(done)** → resolver core **(done)** → session-scoped `endpoints/`+`cluster_hostname` name source **(done)** → per-session `ClusterDNSServer` wiring + resolv.conf **(done)** → remove the peer map from `/etc/hosts`, localhost+self only, resolver-only like Docker **(done, live-validated 2-node — multi-node via etcd names, single-node via registered cluster_host_ips)**. Dynamic membership + backend-unified peer resolution achieved. |
...
| 2026-08-11 | **Overlay encryption = kernel IPSec (ESP/AES-GCM) on the VXLAN tunnel, not userspace crypto; one session key distributed via etcd meta (no IKE), XFRM programmed per peer beside the FDB.** | Packet crypto must be kernel/hardware (AES-NI) to keep the fabric at line rate — Python per-packet would cap it at a few Mbps. Transport-mode ESP between VTEPs keeps the L2 overlay unchanged (Docker Swarm's model), unlike WireGuard (L3). A single session key read from the meta (like the VNI) needs no IKE/key-exchange component, and a deterministic SPI per ordered VTEP pair lets both ends agree without a handshake — consistent with 'reuse etcd, no new coordination'. See [overlay-encryption](./BEP-1062/overlay-encryption.md). |
...
| 2026-08-06 | **Cluster names are session-scoped, not a global namespace; the name source is the `endpoints/` table enriched with `cluster_hostname`, and the resolver is one instance per session bound to that session's gateway.** Resolves the `endpoints/`-vs-`hosts/`-projection open question toward enriching `endpoints/` | Every session names kernels identically (`main1`/`sub1`/…), so names are unique only within a session — moving them to etcd must not flatten them into a global keyspace. It doesn't: etcd is already partitioned `network/session/{session_id}/…`, the coordinator watches per session, and privnet allocates a distinct LOCAL gateway per session, so `resolve_cluster_name(session_id, hostname)` keys on the session and different sessions' identical names are answered by different resolver instances — the same session-locality the per-container static `/etc/hosts` had. The map is built from the **full** endpoint table (own + remote), not the remote-only FDB view, so a co-located peer resolves too. `cluster_hostname` is nullable on the wire (`EndpointAddr.to/from_etcd_payload`) for backward compatibility. |
...
- Cluster name resolution: resolver TTL/cache cadence, and TCP (large-response) fallback. (The `endpoints/`-vs-`hosts/`-projection and naming-scope questions are settled — see 2026-08-06 Decision Log.) See [cluster-name-resolution](./BEP-1062/cluster-name-resolution.md).

...
# Expose the current Backend.AI client API version as the |latest_api_version|
...
# Named after the manager's `LATEST_API_VERSION` rather than a bare
# `api_version`, which already appears in docs as a session-template field
# and an OpenAPI schema property and would read ambiguously as a global
# substitution.
...
rst_prolog = f"""
.. |latest_api_version| replace:: {latest_api_version}
"""

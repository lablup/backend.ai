...
# Expose the current Backend.AI client API version as a global substitution
# so docs do not hard-code the string. Source of truth:
...
# The substitution name is deliberately ugly: `rst_prolog` injects it into
# every document, and a plain `|api_version|` would collide with the
# `api_version` that already appears in the docs as a session-template
# field and an OpenAPI schema property.
...
latest_api_version = _read_api_version()
rst_prolog = f"""
.. |_latest_api_version_placeholder| replace:: {latest_api_version}
"""

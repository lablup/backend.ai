...
- **GPU 워크로드는 대부분의 VM 최적화 기법을 무력화한다**: Firecracker는 GPU 패스스루 불가, VM 풀링은 GPU 낭비, GPU 초기화 오버헤드로 인해 Cold Start가 30초~수 분. 단, 멀티 GPU는 `pcie_root_port` 설정으로 지원 가능 (NVIDIA GPU Operator 우회 시).
...
- VFIO 패스스루를 통해 **단일 GPU 지원**
- **멀티 GPU 지원** — `pcie_root_port` 설정으로 여러 VFIO 디바이스를 단일 VM에 패스 가능. 단, NVIDIA GPU Operator 경로는 단일 GPU만 지원하므로, Backend.AI처럼 자체 GPU 관리를 하는 경우에만 멀티 GPU 가능. QEMU가 Cloud Hypervisor보다 안정적
...
| 단일 GPU | 완전 지원 | 지원 | 지원 |
| 인스턴스당 멀티 GPU | 완전 지원 | 지원 (다중 VFIO, `pcie_root_port` 설정 필요, GPU Operator 우회 시) | 지원 (다중 VFIO) |
...
| GPU 집약적 분산 학습 | DooD 선호 (성능), Kata도 멀티 GPU 가능 |

...
| 2 | 실 동작 테스트 | 요청이 실제로 그렇게 처리되는가 | BA-7489 | 대기 |
| 3 | 권한 검사 로직 확인 | 검사되는 권한 자체가 옳은가 | 미생성 | 대기 |

1부는 서버를 띄우지 않고 코드만 읽는다. 2부는 살아 있는 매니저에 요청을 보낸다. 3부는
`backend.ai mgr ops list`와 `backend.ai mgr ops entities`의 출력을 RBAC 코드(permission,
scope, entity type)와 대조해 로직 자체를 검토한다. 세 조사 모두 수정은 하지 않는다 — 발견은
각각 별도 이슈로 낸다.
...
`G`는 3부에서 게이트 규칙 전체를 볼 때 같이 판단한다.

...
_EXPORT_SERVER_SUBAPP_MODULES = (_auth_api, _export_api)


@asynccontextmanager
async def _export_domain_ctx(root_ctx: RootContext) -> AsyncIterator[None]:
    """Set up repositories and processors for export-domain component tests."""
...
def server_subapp_pkgs() -> list[str]:
    """Load only the subapps required for export-domain tests."""
    return [".auth", ".export"]


@pytest.fixture()
def server_cleanup_contexts() -> list[CleanupContext]:
    """Provide cleanup contexts for export-domain component tests."""
    return [
        redis_ctx,
        database_ctx,
        monitoring_ctx,

...
@asynccontextmanager
async def _rbac_domain_ctx(root_ctx: RootContext) -> AsyncIterator[None]:
    """Set up repositories and processors for RBAC-domain component tests.
...
def server_subapp_pkgs() -> list[str]:
    """Load only the subapps required for RBAC-domain tests."""
    return [".auth", ".rbac"]


@pytest.fixture()
def server_cleanup_contexts() -> list[CleanupContext]:
    """Provide cleanup contexts for RBAC-domain component tests.
    ...
    - _rbac_domain_ctx: repositories and processors wired with real clients
    """
    return [
        redis_ctx,
        database_ctx,

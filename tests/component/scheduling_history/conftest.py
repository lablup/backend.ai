...
def server_subapp_pkgs() -> list[str]:
    """Load only the subapps required for scheduling-history domain tests."""
    return [".auth", ".scheduling_history"]


@pytest.fixture()
def server_cleanup_contexts() -> list[CleanupContext]:
    """Provide cleanup contexts for scheduling-history domain component tests.
    ...
    - _scheduling_history_domain_ctx: repositories and processors wired with real clients
    """
    return [
        redis_ctx,
        database_ctx,
        ...
        event_hub_ctx,
        background_task_ctx,
        _scheduling_history_domain_ctx,
    ]

"""Component tests for storage namespace lifecycle.

Tests register/unregister integration via bucket APIs, and
register, unregister, per-storage listing, search with pagination,
and all-namespaces grouped query through the StorageNamespaceProcessors
...
class TestStorageNamespace:
    ...
    async def test_unregister_namespace(
        ...
        assert resolved.entity_id() == ns["id"]
...
class TestStorageNamespaceSearch:
    ...
    async def test_search_namespaces(
        ...
        """Search namespaces returns matching results."""
        storage = await object_storage_factory()

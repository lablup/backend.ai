...
_QUOTA_SCOPE_SERVER_SUBAPP_MODULES = (_auth_api, _quota_scope_api)


@asynccontextmanager
async def _quota_scope_domain_ctx(root_ctx: RootContext) -> AsyncIterator[None]:
    """Set up repositories and processors for quota-scope-domain component tests.
    ...
    The storage_manager has no proxies configured in the test environment,
    so only permission enforcement and search (which handles empty proxy lists
    gracefully) are fully testable without a real storage proxy.
    """
...
def server_cleanup_contexts() -> list[CleanupContext]:
    """Provide cleanup contexts for quota-scope-domain component tests.
    ...
    - _quota_scope_domain_ctx: repositories and processors wired with real clients
    """
    return [
        redis_ctx,
        database_ctx,

...
**G9의 기대가 `/etc/hosts` → 이름 해석으로 바뀌었다.** `f51f3d6038`(resolver-only peer names)이
피어 맵을 `/etc/hosts`에서 걷어냈다. 이제 그 파일에는 localhost와 커널 자신의 이름만 있고, 피어는
세션 리졸버(Docker 백엔드에서는 dockerd 내장 DNS)로 해석된다. 실측(3커널 단일노드 세션):
`getent hosts main1/sub1/sub2` 모두 성공, `/etc/hosts`에는 `172.20.0.2 sub2` 한 줄뿐.
파일 내용을 검사하던 기존 기대는 이미 제거된 계약을 검사하고 있었다.
...
| G9 | **단일노드 클러스터 세션** (`cluster_size>1` + `SINGLE_NODE`) | 피어가 노드 로컬 /26에 **결정론적으로** 배치됨. 모든 커널에서 전체 피어가 **이름으로 resolve**되고 자기 호스트명이 `127.0.1.1`로 덮이지 않음. `BACKENDAI_CLUSTER_HOSTS` 전 커널 동일. 전 커널쌍 연결성 | D+I | M6 (`a4a3414e6`, `16401871d`, `e23bb2c51`) — 2노드와 **완전히 다른 경로**이고 결함이 가장 많이 나온 곳 | 🔴회귀 ⬜ |
| G20 | 암호화 세션의 언더레이 캡처 | VTEP 인터페이스에서 **ESP만** 관측, 평문 VXLAN 0 | **D** | 도달성 테스트로는 구분 불가 — 정책이 빠져도 통신은 그대로 된다 | ⬜ |
| G21 | OUTPUT mark 제거 후 통신 시도 | 평문 VXLAN 0 (**fail-closed**). 트래픽이 멈추지 새어나가지 않음 | **D** | MARK는 terminating이 아님. netns 실측: mark 제거 시 8/8 프레임이 평문 유출 | ⬜ |
| G22 | INPUT 최상단에 남의 규칙 삽입 | 우리 jump가 다시 1번으로 복구됨 | I | `iptables -C`는 "존재"만 답한다 — 위에 ACCEPT가 들어가면 통과하면서 우회된다 | ⬜ |
| G23 | 우리 chain `-F` | drift 재확인이 규칙을 다시 채움 | I | 방화벽 리로드 사이에 수신측이 다시 열린다 | ⬜ |

...
class TestRBACCreator:
    ...
    async def test_create_order(
        ...
        """Test that _create is called before create_entity"""
        create_order: list[str] = []

        class OrderTrackingEntityCreator(RBACEntityCreator):
            def scope_id(self) -> ScopeId:
                return test_scope_id

            def object_id(self) -> ObjectId:
                return test_object_id

            async def create_entity(self, db_session: SASession) -> None:
                create_order.append("entity")
                await super().create_entity(db_session)

        class OrderTrackingRBACCreator(RBACCreator[str]):
            async def _create(self, db_session: SASession) -> str:
                create_order.append("main")
                return "test-value"

        entity_creator = OrderTrackingEntityCreator()
        rbac_creator = OrderTrackingRBACCreator(entity_creator)

        async with database_engine.begin_session() as db_sess:
            await rbac_creator.create(db_sess)
            await db_sess.flush()

            # Verify order: _create should be called before create_entity
            assert create_order == ["main", "entity"]
...
class TestScopeIdAndObjectId:
    """Test cases for ScopeId and ObjectId"""

    @pytest.mark.asyncio
    async def test_scope_id_serialization(self) -> None:
        """Test ScopeId to_str and from_str methods"""
        scope_id = ScopeId(
            scope_type=ScopeType.DOMAIN,
            scope_id="test-domain-123",
        )

        serialized = scope_id.to_str()
        assert serialized == "domain:test-domain-123"

        deserialized = ScopeId.from_str(serialized)
        assert deserialized.scope_type == scope_id.scope_type
        assert deserialized.scope_id == scope_id.scope_id

    @pytest.mark.asyncio
    async def test_object_id_serialization(self) -> None:
        """Test ObjectId to_str and from_str methods"""
        object_id = ObjectId(
            entity_type=EntityType.USER,
            entity_id="user-456",
        )

        serialized = object_id.to_str()
        assert serialized == "user:user-456"

        deserialized = ObjectId.from_str(serialized)
        assert deserialized.entity_type == object_id.entity_type
        assert deserialized.entity_id == object_id.entity_id

    @pytest.mark.asyncio
    async def test_scope_id_all_types(self) -> None:
        """Test ScopeId with all scope types"""
        for scope_type in ScopeType:
            scope_id = ScopeId(scope_type=scope_type, scope_id=f"test-{scope_type}")
            serialized = scope_id.to_str()
            deserialized = ScopeId.from_str(serialized)

            assert deserialized.scope_type == scope_type
            assert deserialized.scope_id == f"test-{scope_type}"

    @pytest.mark.asyncio
    async def test_object_id_all_types(self) -> None:
        """Test ObjectId with all entity types"""
        for entity_type in EntityType:
            object_id = ObjectId(entity_type=entity_type, entity_id=f"test-{entity_type}")
            serialized = object_id.to_str()
            deserialized = ObjectId.from_str(serialized)

            assert deserialized.entity_type == entity_type
            assert deserialized.entity_id == f"test-{entity_type}"

...
class TestContainerRegistryRepository:
    ...
    async def test_modify_registry_success(
        ...
        assert result.ssl_verify == registry_for_modification.ssl_verify
    ...
    async def test_delete_registry_success(
        ...
        # When
        result = await repository.delete_registry(sample_registry.id)
        ...
        with pytest.raises(ContainerRegistryNotFound):
            await repository.get_by_registry_and_project(
                sample_registry.registry_name, sample_registry.project
            )
    ...
    async def test_delete_registry_with_images(
        ...
        with pytest.raises(ContainerRegistryNotFound):
            await repository.get_by_registry_and_project(registry.registry_name, registry.project)

    @pytest.fixture
    async def groups_for_registry_creation(
        self, test_groups_factory
    ) -> AsyncGenerator[list[UUID], None]:
        """Pre-created groups for registry creation with allowed_groups testing."""
    ...
    async def test_create_registry_with_allowed_groups(
        ...
        repository: ContainerRegistryRepository,
        database_engine: ExtendedAsyncSAEngine,
        groups_for_registry_creation: list[UUID],
    ) -> None:
        ...
        try:
            ...
            result = await repository.create_registry(
                ContainerRegistryCreator(
                    ...
                    allowed_groups=AllowedGroupsModel(
                        add=[
                            str(groups_for_registry_creation[0]),
                            str(groups_for_registry_creation[1]),
                        ],
                        remove=[],
                    ),
                )
            ...
            async with database_engine.begin_readonly_session() as session:
                ...
                assert {a.group_id for a in associations} == set(groups_for_registry_creation)
        finally:
            # Cleanup
            async with database_engine.begin_session() as session:
                await session.execute(
        ...
        result = await repository.modify_registry(
            ...
            ContainerRegistryModifier(
                ...
                allowed_groups=TriState.update(
                    AllowedGroupsModel(
                        add=[str(g) for g in registry_and_groups_for_adding.group_ids], remove=[]
                    )
                ),
            ),
        )
    ...
    async def test_modify_registry_remove_nonexistent_allowed_groups(
        ...
        # Given - A modifier attempting to remove a non-existent group
        modifier = ContainerRegistryModifier(
            url=OptionalState.nop(),
            type=OptionalState.nop(),
            registry_name=OptionalState.nop(),

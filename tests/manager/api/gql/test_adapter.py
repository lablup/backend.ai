...
class TestBaseGQLAdapterBuildPagination:
    ...
    @pytest.fixture
    def mock_condition(self) -> MagicMock:
        """Create a mock cursor condition."""
        return MagicMock()
    ...
    def test_build_pagination_cursor_without_condition_error(
        self, adapter: BaseGQLAdapter, mock_order: MagicMock
    ) -> None:
        """Test that cursor pagination without cursor_condition raises error."""
        with pytest.raises(InvalidGraphQLParameters) as exc_info:
            adapter.build_pagination(
                first=10,
                after="some-cursor",
                default_order=mock_order,
                # cursor_condition is missing
            )
        assert "cursor_condition and default_order are required" in str(exc_info.value)

    def test_build_pagination_cursor_without_order_error(
        self, adapter: BaseGQLAdapter, mock_condition: MagicMock
    ) -> None:

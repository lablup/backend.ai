"""preset 읽기 — id로, 그리고 여러 id로. 둘 다 인증만 본다.

여러 id로 읽기는 훑기를 id 조건으로 부른 것이라, 없는 id가 거부가 아니라 빈 자리로 온다.
"""
...
class AUserGrantedNothingReadsById(
    ...
    def describe(self) -> str:
        return "아무 권한도 받지 않은 사용자가 id로 조회하면, 그 preset 전체가 온다. 이 읽기는 인증만 본다"
...
class AnIdNothingAnswersToIsNotFound(
    ...
    @override
    def describe(self) -> str:
        return "아무 preset도 갖지 않은 id로 조회하면 대상이 없다는 것으로 거부된다"

"""비밀 상태 읽기 — 저장된 비밀 키를 어느 키가 쥐고 있는지 세고, 누가 읽을 수 있는가.

세는 비밀은 키 쌍의 비밀 키 하나뿐이고 부르는 사람도 키 쌍을 가지므로, 집계에는 언제나 부르는
사람의 것이 들어 있다.
"""
...
class TheSuperadminCountsPlaintextSecrets(
    ...
    def describe(self) -> str:
        return (
            "쓰기 제공자가 평문일 때 슈퍼관리자가 상태를 읽으면, 쓰기 제공자는 평문이고 집계는 "
            "부르는 사람까지 평문 비밀 키를 쥔 수 한 줄이다"
        )
...
class MixedKeysAreCountedApart(
    ...
    def describe(self) -> str:
        return (
            "설정 키로 암호화된 비밀 키와 평문 비밀 키가 섞여 있을 때 상태를 읽으면, "
            "그 키가 쥔 수와 평문인 수가 두 줄로 따로 온다"
        )
...
class TheMonitorReadsTheStatusLikeTheSuperadmin(
    ...
    @override
    def describe(self) -> str:
        return "모니터 역할이 상태를 읽으면 슈퍼관리자와 같은 답이 온다. 전역 역할 문은 모니터의 읽기를 지나게 한다"
...
class AUserWhoIsNotTheSuperadminMayNotReadTheStatus(
    ...
    @override
    def describe(self) -> str:
        return "슈퍼관리자가 아닌 사용자가 상태를 읽으면 역할로 거부된다"

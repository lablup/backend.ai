...
The only place in the definition scenarios that knows how the app config processors are
...
from ai.backend.manager.api.adapters.app_config_definition.adapter import AppConfigDefinitionAdapter
...
async def adapter(
    ...
    return AppConfigDefinitionAdapter(
        AppConfigProcessors(
            ...
            AppConfigService(repository, AppConfigRepository(provider)),
        )
    )

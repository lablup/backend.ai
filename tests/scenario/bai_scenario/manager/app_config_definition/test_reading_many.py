...
class LoadingByIds(When[TwoDefinitionsAndACaller, AppConfigDefinitionAdapter, Loaded]):
    ...
    async def call(
        ...
        asked = (
            [] if self.nothing else [laid.first.id, laid.second.id, AppConfigDefinitionID(uuid4())]
        )
...
@dataclass(frozen=True)
class OneNodeAndTwoRefused(Then[TwoDefinitionsAndACaller, Loaded]):
        ...
        return [
            *seen,
            Held[object](
                ...
                SameAs[object](laid.first.id, "심은 첫째 정의"),
            ),
        ...
        return [
            *seen,
            ...
            Held[object](
                ...
                SameAs[object](laid.second.id, "심은 둘째 정의"),
            ),
...
class MixedIdsAreAnsweredEach(
    ...
    def describe(self) -> str:
        return (
            "정의 둘 중 첫째에만 읽기 권한을 받은 사용자가 그 둘과 없는 id 하나를 한 번에 "
    ...
    def given(self) -> Given[SeedingSession, TwoDefinitionsAndACaller]:
        return TwoDefinitionsAndSomeone(reads_first=True)
...
class TheSuperadminSeesBoth(
    ...
    def describe(self) -> str:
        return (
            "슈퍼관리자가 정의 둘과 없는 id 하나를 한 번에 읽으면, 둘은 노드로 오고 없는 id "

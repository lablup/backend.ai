...
class TestDomainLifecycleInfo:
    ...
    def test_creation_with_all_fields(self) -> None:
        ...
        info = DomainLifecycleInfo(
            is_active=True, is_default=False, created_at=now, modified_at=now, updated_at=now
        )
    ...
    def test_inactive_domain(self) -> None:
        now = datetime.now(tz=UTC)
        info = DomainLifecycleInfo(
            is_active=False, is_default=False, created_at=now, modified_at=now, updated_at=now
        )

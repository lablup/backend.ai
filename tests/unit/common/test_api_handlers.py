...
# ---------------------------------------------------------------------------
# Regression: POST handler must use BodyParam, not QueryParam (BA-5694)
# ---------------------------------------------------------------------------


class TestPostBodyRequiredModel(BaseRequestModel):
    image_name: str
    visibility: str | None = Field(default="user")


class TestPostBodyRequiredResponse(BaseResponseModel):
    image_name: str
    visibility: str


class TestPostBodyRequiredHandler:
    """Handler that correctly uses BodyParam for a POST endpoint."""

    @api_handler
    async def handle_post(self, body: BodyParam[TestPostBodyRequiredModel]) -> APIResponse:
        parsed = body.parsed
        return APIResponse.build(
            status_code=HTTPStatus.OK,
            response_model=TestPostBodyRequiredResponse(
                image_name=parsed.image_name, visibility=parsed.visibility or "user"
            ),
        )


class TestPostQueryWrongHandler:
    """Handler that incorrectly uses QueryParam for a POST endpoint (the bug)."""

    @api_handler
    async def handle_post(self, query: QueryParam[TestPostBodyRequiredModel]) -> APIResponse:
        parsed = query.parsed
        return APIResponse.build(
            status_code=HTTPStatus.OK,
            response_model=TestPostBodyRequiredResponse(
...
async def test_post_body_param_parses_json_body(aiohttp_client: Any) -> None:
    """BodyParam on a POST endpoint should parse the JSON body correctly."""
    handler = TestPostBodyRequiredHandler()
    app = web.Application()
    app.router.add_post("/test", handler.handle_post)
    client = await aiohttp_client(app)

    resp = await client.post("/test", json={"image_name": "my-image", "visibility": "project"})

    assert resp.status == HTTPStatus.OK
    data = await resp.json()
    assert data["image_name"] == "my-image"
    assert data["visibility"] == "project"


async def test_post_query_param_fails_for_json_body(aiohttp_client: Any) -> None:
    """QueryParam on a POST endpoint ignores the JSON body, causing validation failure."""
    handler = TestPostQueryWrongHandler()
    app = web.Application()
    app.router.add_post("/test", handler.handle_post)
    client = await aiohttp_client(app)
    ...
    # QueryParam reads from query string, not body — required field 'image_name' is missing

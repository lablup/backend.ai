...
class TestMultipleAgentsConfigValidation:
    ...
    def test_agent_overrides_resource_config(
        ...
        assert agent_configs[0].resource.allocations.mem == 8 * (1024**3)
        assert agent_configs[1].resource.allocations is not None
        assert agent_configs[1].resource.allocations.mem == 4 * (1024**3)
    ...
    def test_multiple_agents_with_mixed_overrides(
        ...
        assert agent_configs[0].agent.kernel_creation_concurrency == 8
        assert agent_configs[0].resource.allocations is not None
        assert agent_configs[0].resource.allocations.mem == 16 * (1024**3)
        assert agent_configs[1].agent.kernel_creation_concurrency == 4
        assert agent_configs[1].resource.allocations is not None
        assert agent_configs[1].resource.allocations.mem == 8 * (1024**3)
...
class TestResourceAllocationModes:
    ...
    def test_shared_mode_rejects_allocations(
        self,
        default_raw_config: RawConfigT,
    ) -> None:
        raw_config = {
    ...
    def test_auto_split_mode_rejects_allocations(
        self,
        default_raw_config: RawConfigT,
    ) -> None:
        raw_config = {
    ...
    def test_manual_mode_requires_cpu(
        self,
        default_raw_config: RawConfigT,
    ) -> None:
        raw_config = {
        ...
        assert "must specify cpu" in str(exc_info.value)

    def test_manual_mode_valid_device_based_configuration(
        self,
        default_raw_config: RawConfigT,
    ) -> None:
        raw_config = {
        ...
        agent_configs = config.get_agent_configs()
        assert agent_configs[0].resource.allocations is not None
        assert agent_configs[0].resource.allocations.cpu == [
            DeviceId("0"),
            DeviceId("1"),
            DeviceId("2"),
            DeviceId("3"),
        ]
        assert agent_configs[0].resource.allocations.devices[DeviceName("cuda")] == [
            DeviceId("cuda0"),
            DeviceId("cuda1"),
        ]
        assert agent_configs[1].resource.allocations is not None
        assert agent_configs[1].resource.allocations.cpu == [
            DeviceId("4"),
            DeviceId("5"),
            DeviceId("6"),
            DeviceId("7"),
        ]
        assert agent_configs[1].resource.allocations.devices[DeviceName("cuda")] == [
            DeviceId("cuda2"),
            DeviceId("cuda3"),
        ]

    def test_manual_mode_agents_with_different_device_types_allowed(
        self,
        default_raw_config: RawConfigT,
    ) -> None:
        raw_config = {
        ...
        agent_configs = config.get_agent_configs()
        assert agent_configs[0].resource.allocations is not None
        assert DeviceName("cuda") in agent_configs[0].resource.allocations.devices
        assert agent_configs[1].resource.allocations is not None
        assert DeviceName("cuda") not in agent_configs[1].resource.allocations.devices

    def test_manual_mode_rejects_old_slot_based_format(
        self,
        default_raw_config: RawConfigT,
    ) -> None:
        raw_config = {
        ...
        assert "Old slot-based format detected" in str(exc_info.value)

    def test_manual_mode_rejects_string_numeric_value(
        self,
        default_raw_config: RawConfigT,
    ) -> None:
        raw_config = {
            ...
            "agents": [
                {
                    ...
                    "resource": {
                        "cpu": ["0", "1"],
                        "mem": "32G",
                        "devices": {"cuda": "0.5"},  # Old slot-based format
                    },
                },
                {
                    "agent": {"id": "agent-2"},
    ...
    def test_manual_mode_rejects_cpu_in_devices(
        self,
        default_raw_config: RawConfigT,
    ) -> None:
        raw_config = {
        ...
        assert "CPU must be specified via 'cpu' field" in str(exc_info.value)

    def test_manual_mode_rejects_old_integer_cpu_format(
        self,
        default_raw_config: RawConfigT,
    ) -> None:
        raw_config = {
        ...
        assert "Old integer format detected for 'cpu'" in str(exc_info.value)

    def test_manual_mode_cpu_only_is_valid(
        self,
        default_raw_config: RawConfigT,
    ) -> None:
        raw_config = {

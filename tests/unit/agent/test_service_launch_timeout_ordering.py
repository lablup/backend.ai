...
# The budget the kernel runner grants a service port. It lives in a package that cannot be
# imported the other way round, since that one ships inside the container, so the ordering
# between the two is only ever checked here.
...
class TestStartServiceReplyBudget:
    ...
    async def test_reports_a_timeout_only_once_the_runner_has_had_its_full_budget(
        ...
        monkeypatch.setattr(agent_kernel, "SERVICE_START_REPLY_TIMEOUT_SEC", _REPLY_DELAY_SEC)

...
class TestCleanKernelCollectsLogs:
    ...
    def _agent(self, events: list[str], monkeypatch: pytest.MonkeyPatch) -> ContainerdAgent:
        ...
        agent._session_network = cast(
            Any,
            SimpleNamespace(
                remove_container=remove_container,
                port_publisher=lambda: SimpleNamespace(remove_container=_async_return([])),
            ),
        )
...
class TestCgroupPath:
    ...
    async def test_v2_reads_the_unified_hierarchy(self, monkeypatch: pytest.MonkeyPatch) -> None:
        agent = self._agent(monkeypatch, version="2")
        path = await agent.get_cgroup_path(CgroupController.MEMORY, ContainerId("kern-1"))
        assert path == Path("/sys/fs/cgroup/backend-ai/kern-1")

    async def test_v1_reads_the_controllers_own_hierarchy(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setattr(
            agent_mod, "get_cgroup_mount_point", lambda v, c: Path(f"/sys/fs/cgroup/{c.value}")
        )
        agent = self._agent(monkeypatch, version="1")

        assert await agent.get_cgroup_path(CgroupController.MEMORY, ContainerId("kern-1")) == Path(
            "/sys/fs/cgroup/memory/backend-ai/kern-1"
        )
        assert await agent.get_cgroup_path(CgroupController.CPUACCT, ContainerId("kern-1")) == Path(
            "/sys/fs/cgroup/cpuacct/backend-ai/kern-1"
        )

    async def test_a_missing_v1_hierarchy_does_not_raise(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        ...
        def no_such_hierarchy(version: str, controller: CgroupController) -> Path:
        ...
        assert await agent.get_cgroup_path(CgroupController.BLKIO, ContainerId("kern-1")) == Path(
            "/sys/fs/cgroup/backend-ai/kern-1"
        )
...
class TestEnumerateContainerPids:
    ...
    def _agent(
        ...
        async def _cgroup_path(controller: CgroupController, cid: ContainerId) -> Path:
            return cgroup_dir

        monkeypatch.setattr(agent, "get_cgroup_path", _cgroup_path)
        monkeypatch.setattr(agent, "get_cgroup_version", lambda: version)

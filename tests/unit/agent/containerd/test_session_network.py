...
_BRIDGE_NC = {"backend": "bridge", "subnet": "10.129.0.0/24"}
...
class TestMetaParsing:
    ...
    def test_bridge_has_no_vni_and_default_mtu(self) -> None:
        meta = session_net_meta_from_network_config("s2", _BRIDGE_NC)
...
class TestEnsureSession:
    ...
    async def test_bridge_self_member_has_no_vtep(self) -> None:
        etcd, backend, runner = FakeEtcd(), RecordingBackend(), RecordingRunner()
        facade = _facade(etcd, backend, runner)
...
class TestBackendResolution:
    async def test_selects_backend_by_session_config(self) -> None:
        ...
        vxlan_b, bridge_b = RecordingBackend(), RecordingBackend()
        facade = _facade(etcd, vxlan_b, runner, backends={"vxlan": vxlan_b, "bridge": bridge_b})
        await facade.ensure_session("sv", "k1", _VXLAN_NC)
        await facade.ensure_session("sh", "k1", _BRIDGE_NC)
        try:
            assert vxlan_b.setup == ["sv"]  # vxlan config -> vxlan backend only
            assert bridge_b.setup == ["sh"]  # bridge config -> bridge backend only
        finally:
    ...
    async def test_unknown_backend_raises(self) -> None:
        ...
        with pytest.raises(UnknownNetworkBackend):
            await facade.ensure_session("s1", "k1", _BRIDGE_NC)  # bridge not registered
...
@pytest.mark.parametrize("nc", [_VXLAN_NC, _BRIDGE_NC])

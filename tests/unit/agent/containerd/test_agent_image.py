...
from ai.backend.agent.network.runtime import ContainerInfo, ImageInfo, TaskEvent
...
class FakeFacade:
    def __init__(
        ...
        *,
        ...
        # The publisher the session network hands out; `_agent` installs the test's own.
...
class _FakeRuntime:
    ...
    async def create_task(self, container_id: str, *, use_logger: bool = True) -> Any:
        return None
...
def _agent(
    ...
    facade.publisher = port_forwarder or _NoForwards()
    return agent
...
class TestCgroupPath:
    async def test_cgroup_path_matches_spec_cgroups_path(self) -> None:
        ...
        agent = _agent(FakeFacade())
        path = await agent.get_cgroup_path(CgroupController.MEMORY, ContainerId("abc123"))

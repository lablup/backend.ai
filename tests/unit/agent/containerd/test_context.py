...
from ai.backend.agent.network.orchestrator import LaunchResult
from ai.backend.agent.network.runtime import ExecResult, TaskHandle
...
class FakeFacade:
    def __init__(self, exec_exit_code: int = 0) -> None:
        ...
        # The publisher the session network hands out; a test that cares installs its own.
        self.publisher: Any = FakePortForwarder()

    def port_publisher(self) -> Any:
...
def _context(
    ...
    *,
    ...
    if port_forwarder is not None:
        facade.publisher = port_forwarder
    ...
    ctx.local_config = cast(
        ...
        SimpleNamespace(
            container=SimpleNamespace(
                ...
                # The scratch handover reads these whether or not the agent is root: a rootless
                # one hands the files over through a user namespace instead of returning early.
...
class TestDomainSocketProxies:
    ...
    def _ctx(self, tmp_path: Path, sockets: list[str]) -> Any:
        ...
        # A short base: the proxy socket lives under it, and a unix socket path is capped at 107
        # bytes -- pytest's own tmp_path plus the token-named socket runs over that.
        ipc_base = Path(tempfile.mkdtemp(prefix="bai-ipc-", dir="/tmp"))
        ctx.local_config = cast(
            Any,
            SimpleNamespace(
                agent=SimpleNamespace(ipc_base_path=ipc_base),
                container=SimpleNamespace(
                    scratch_root=tmp_path,
                    scratch_type=ScratchType.HOSTDIR,
                    deeplearning_samples_path=None,
...
class TestTheKernelUserOwnsItsHome:
    ...
    def _ctx(
        ...
        *,
        ...
        # A rootless agent reaches the same files by another route; record both the same way so a
        # test can say what was handed over without caring which route took it there.
        monkeypatch.setattr(
            "ai.backend.agent.containerd.agent.chown_via_userns",
            lambda paths, uid, gid, **_: chowned.extend((Path(p), uid, gid) for p in paths),
        )
        ctx = _context(FakeFacade())
    ...
    async def test_a_rootless_agent_hands_over_the_same_files(
        self, tmp_path: Path, monkeypatch: Any
    ) -> None:
        # It has no CAP_CHOWN, so it goes through a user namespace instead of chowning directly.
        # Handing over nothing was survivable only while the container's identity happened to be
        # the agent's own uid; the moment it is not, the user cannot write to its own home.
        ctx, chowned = self._ctx(tmp_path, monkeypatch, root=False)
        await ctx.prepare_scratch()

        work_dir = tmp_path / "kern-123" / "work"
        owned = {path for path, _uid, _gid in chowned}
        assert work_dir in owned
        assert work_dir / ".bashrc" in owned
        assert all(uid == 1000 and gid == 1001 for _p, uid, gid in chowned)
...
class TestTheClusterKeyIsReadableByTheKernelUser:
    ...
    async def test_a_rootless_agent_hands_over_the_key_too(
        self, tmp_path: Path, monkeypatch: Any
    ) -> None:
        # 0600 is exactly the mode a rootless agent used to leave unreadable by the kernel's user.
        chowned = await self._prepare(tmp_path, monkeypatch, root=False)
        owned = {path for path, _u, _g in chowned}
        assert tmp_path / "config" / "ssh" / "id_cluster" in owned

...
def _plan() -> EndpointPlan:
    return EndpointPlan(
        attachments=[
            NetworkAttachSpec(
                kind=AttachKind.CNI, interface_name="eth0", role=NetworkRole.LOCAL,
                is_default_route=True, cni_config={"type": "bridge"},
            ),
            NetworkAttachSpec(
...
class TestContainerNetworkProvisioner:
    async def test_attach_builds_plan_and_applies_cni_with_pid_netns(self) -> None:
        ...
        plan = await prov.attach(
            cast(KernelCreationConfig, {}), cast(ClusterInfo, {}),
            meta=_META, container_id="c1", task_pid=4242,
        )

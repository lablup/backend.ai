...
class TestLoginClientTypeService:
    ...
    async def test_create(
        ...
        mock_repository.create = AsyncMock(return_value=data)

        creator = MagicMock(spec=Creator)
        action = CreateLoginClientTypeAction(creator=creator)
        result = await service.create(action)
    ...
    async def test_create_duplicate_name_raises(
        ...
        mock_repository.create = AsyncMock(side_effect=LoginClientTypeConflict("already exists"))

        creator = MagicMock(spec=Creator)
        action = CreateLoginClientTypeAction(creator=creator)

        with pytest.raises(LoginClientTypeConflict):
            await service.create(action)
    ...
    async def test_update(
        ...
        mock_repository.update = AsyncMock(return_value=data)
    ...
    async def test_delete(
        ...
        data = _make_data()
        mock_repository.delete = AsyncMock(return_value=data)

        action = DeleteLoginClientTypeAction(id=data.id)
        result = await service.delete(action)

        assert result.login_client_type == data
        mock_repository.delete.assert_called_once_with(data.id)
...
class TestLoginClientTypeAdminService:
    ...
    async def test_search(
        ...
        mock_admin_repository.search = AsyncMock(
        ...
        action = AdminSearchLoginClientTypesAction(querier=querier)
        result = await admin_service.search(action)

        assert result.items == items
        assert result.total_count == 2
        assert result.has_next_page is False
        assert result.has_previous_page is False
        mock_admin_repository.search.assert_called_once_with(querier=querier)

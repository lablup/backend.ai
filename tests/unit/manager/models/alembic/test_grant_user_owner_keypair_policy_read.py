"""Runs the user_owner keypair_resource_policy READ grant against a real database and reads the rows back."""
...
from ai.backend.manager.models.alembic.versions.f46f55fed5a6_grant_user_owners_read_on_their_keypair_policy import (
    _PRESET_ID,
    grant,
    revoke,
)
...
_USER = "user"
_KEYPAIR_POLICY_READ = ("keypair_resource_policy", Permission.READ)
_USER_POLICY_READ = ("user_resource_policy", Permission.READ)
...
async def seeded(db: ExtendedAsyncSAEngine) -> _Fixture:
    """Two user_owner roles, one already granted keypair_resource_policy READ, and a
    role of another user preset."""
    other_preset = uuid.uuid4()
    plain_role, role_already_reading, other_preset_role = (uuid.uuid4() for _ in range(3))
    async with db.begin_session() as session:
        for preset_id, name in ((_PRESET_UUID, "user_owner"), (other_preset, "other")):
            session.add(
                RolePresetRow(
                    id=preset_id, name=name, scope_type=_USER, auto_assign=True, deleted=False
                )
            )
        await session.flush()
        session.add(
            RolePermissionPresetRow(
                role_preset_id=_PRESET_UUID,
                entity_type="user_resource_policy",
        ...
        for role_id, (entity_type, permission) in (
            (plain_role, _USER_POLICY_READ),
            (role_already_reading, _KEYPAIR_POLICY_READ),
        ):
            session.add(
...
class TestGrantUserOwnerKeypairPolicyRead:
    async def test_the_preset_gains_keypair_policy_read(
        self, db: ExtendedAsyncSAEngine, seeded: _Fixture
    ) -> None:
        await _run(db, grant)

        assert await _preset_grants(db) == {_USER_POLICY_READ, _KEYPAIR_POLICY_READ}

    async def test_every_role_of_the_preset_gains_keypair_policy_read(
        self, db: ExtendedAsyncSAEngine, seeded: _Fixture
    ) -> None:
        await _run(db, grant)

        assert await _grants(db, seeded.plain_role) == {_USER_POLICY_READ, _KEYPAIR_POLICY_READ}
        assert await _grants(db, seeded.role_already_reading) == {_KEYPAIR_POLICY_READ}
    ...
    async def test_downgrade_takes_the_grant_back(
        ...
        assert await _preset_grants(db) == {_USER_POLICY_READ}
        assert await _grants(db, seeded.plain_role) == {_USER_POLICY_READ}

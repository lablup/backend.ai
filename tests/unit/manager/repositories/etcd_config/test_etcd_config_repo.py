...
class TestEtcdConfigRepository:
    ...
    async def _seed_agents(self, database: ExtendedAsyncSAEngine, scaling_group: str) -> None:
        ...
        # The uuid is generated by the database when it is not given, and `agent_resources` names
        # the agent by BOTH its id and its uuid -- so the seed has to fix the uuid here rather
        # than let the default mint one it cannot then reference.
        agent_uuids = [AgentUUID(uuid4()) for _ in agent_ids]
        async with database.begin_session() as db_sess:
            ...
            db_sess.add_all([
                ...
                AgentResourceRow(
                    ...
                    agent_uuid=agent_uuids[0],
                    slot_name="mem",
                    capacity=Decimal("32768"),
                ),
                AgentResourceRow(
                    agent_id=agent_ids[1],
                    agent_uuid=agent_uuids[1],
                    slot_name="cpu",
                    capacity=Decimal("16"),
                ),
                AgentResourceRow(

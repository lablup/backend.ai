...
class TestRBACEntityCreatorRelationType:
    """Tests for relation_type parameter in RBAC entity creator."""
    ...
    async def test_default_relation_type_is_auto(
        ...
        """Test that default relation_type is AUTO when not specified."""
        user_id = str(uuid.uuid4())
        ...
        async with database_connection.begin_session() as db_sess:
            ...
            assert creator.relation_type == RelationType.AUTO
    ...
    async def test_explicit_relation_type_ref(
        self,
        database_connection: ExtendedAsyncSAEngine,
        create_tables: None,
    ) -> None:
        """Test creating an entity with explicit RelationType.REF."""
        user_id = str(uuid.uuid4())
    ...
    async def test_relation_type_applied_to_all_scopes(
        self,
        database_connection: ExtendedAsyncSAEngine,
        create_tables: None,
    ) -> None:
        """Test that relation_type is applied to all scope associations."""
        user_id = str(uuid.uuid4())
        project_id = str(uuid.uuid4())

        async with database_connection.begin_session() as db_sess:
            spec = SimpleCreatorSpec(
                name="multi-scope-ref",
                scope_type=ScopeType.PROJECT,
                scope_id=project_id,
            )
            creator: RBACEntityCreator[RBACEntityCreatorTestRow] = RBACEntityCreator(
    ...
    async def test_bulk_creator_default_relation_type_is_auto(
        self,
        database_connection: ExtendedAsyncSAEngine,
        create_tables: None,
    ) -> None:
        """Test that bulk creator default relation_type is AUTO."""
        user_id = str(uuid.uuid4())
        ...
        async with database_connection.begin_session() as db_sess:
            ...
            assoc_rows = (await db_sess.scalars(sa.select(AssociationScopesEntitiesRow))).all()
            assert len(assoc_rows) == 3
            for assoc in assoc_rows:
                assert assoc.relation_type == RelationType.AUTO

    async def test_bulk_creator_explicit_relation_type_ref(
        self,
        database_connection: ExtendedAsyncSAEngine,
        create_tables: None,
    ) -> None:
        """Test bulk creator with explicit RelationType.REF."""
        user_id = str(uuid.uuid4())

...
class TestScopeAdapterBuildQuerier:
    ...
    def test_build_querier_resource_group_with_name_contains(self, adapter: ScopeAdapter) -> None:
        """Test building querier for resource group scope with name contains filter."""
        request = SearchScopesRequest(
            filter=ScopeFilter(
                name=StringFilter(
                    i_contains="rg-",
        ...
        querier = adapter.build_querier(ScopeType.RESOURCE_GROUP, request)

        assert len(querier.conditions) == 1
        assert callable(querier.conditions[0])

    def test_build_querier_resource_group_with_order(self, adapter: ScopeAdapter) -> None:
        """Test building querier for resource group scope with ordering."""
        request = SearchScopesRequest(
            filter=None,
            order=[
                ScopeOrder(
    ...
    def test_build_querier_container_registry_with_name_contains(
        self, adapter: ScopeAdapter
    ) -> None:
        """Test building querier for container registry scope with name contains filter."""
        request = SearchScopesRequest(
            filter=ScopeFilter(
                name=StringFilter(
                    i_contains="docker",
        ...
        querier = adapter.build_querier(ScopeType.CONTAINER_REGISTRY, request)

        assert len(querier.conditions) == 1
        assert callable(querier.conditions[0])

    def test_build_querier_container_registry_with_order(self, adapter: ScopeAdapter) -> None:
        """Test building querier for container registry scope with ordering."""
        request = SearchScopesRequest(
            filter=None,
            order=[
                ScopeOrder(
    ...
    def test_build_querier_artifact_registry_with_name_contains(
        self, adapter: ScopeAdapter
    ) -> None:
        """Test building querier for artifact registry scope with name contains filter."""
        request = SearchScopesRequest(
            filter=ScopeFilter(
                name=StringFilter(
                    i_contains="hf-",
        ...
        querier = adapter.build_querier(ScopeType.ARTIFACT_REGISTRY, request)

        assert len(querier.conditions) == 1
        assert callable(querier.conditions[0])

    def test_build_querier_artifact_registry_with_order(self, adapter: ScopeAdapter) -> None:
        """Test building querier for artifact registry scope with ordering."""
        request = SearchScopesRequest(
            filter=None,
            order=[
                ScopeOrder(

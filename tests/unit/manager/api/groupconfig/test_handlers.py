...
@pytest.fixture
def mock_root_ctx(group_id: uuid.UUID, domain_name: str) -> MagicMock:
    """Create RootContext mock with repositories and processors."""
    ...
    # Mock repositories
    ctx.repositories.group_config.repository.resolve_group_id_and_domain = AsyncMock(
        return_value=(group_id, domain_name)
    )
    ctx.repositories.group_config.repository.check_user_in_group = AsyncMock(return_value=True)
...
class TestCreateDotfile:
    ...
    async def test_create_dotfile_group_not_found(
        ...
        mock_root_ctx.repositories.group_config.repository.resolve_group_id_and_domain.side_effect = ProjectNotFound

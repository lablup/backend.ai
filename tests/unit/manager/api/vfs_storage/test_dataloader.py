"""Tests for vfs_storage GraphQL DataLoader utilities."""
...
class TestLoadVFSStoragesByIds:
    """Tests for load_vfs_storages_by_ids function."""

    @staticmethod
    def create_mock_storage(storage_id: uuid.UUID) -> MagicMock:
        return MagicMock(spec=VFSStorageData, id=storage_id)
    ...
    def create_mock_processor(storages: list[MagicMock]) -> MagicMock:
        ...
        mock_processor.search_vfs_storages.wait_for_complete = AsyncMock(
    ...
    async def test_empty_ids_returns_empty_list(self) -> None:
        ...
        # When
        result = await load_vfs_storages_by_ids(mock_processor, [])

        # Then
        assert result == []
        mock_processor.search_vfs_storages.wait_for_complete.assert_not_called()
    ...
    async def test_returns_storages_in_request_order(self) -> None:
        ...
        # When
        result = await load_vfs_storages_by_ids(mock_processor, [id1, id2, id3])
    ...
    async def test_returns_none_for_missing_ids(self) -> None:
        ...
        # When
        result = await load_vfs_storages_by_ids(mock_processor, [existing_id, missing_id])

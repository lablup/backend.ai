"""Tests for Reservoir registry GraphQL DataLoader utilities."""
...
from ai.backend.manager.api.gql.data_loader.reservoir_registry.loader import (
    load_reservoir_registries_by_ids,
)
from ai.backend.manager.data.reservoir_registry.types import ReservoirRegistryData


class TestLoadReservoirRegistriesByIds:
    """Tests for load_reservoir_registries_by_ids function."""

    @staticmethod
    def create_mock_registry(registry_id: uuid.UUID) -> MagicMock:
        return MagicMock(spec=ReservoirRegistryData, id=registry_id)
    ...
    def create_mock_processor(registries: list[MagicMock]) -> MagicMock:
        ...
        mock_processor.search_reservoir_registries.wait_for_complete = AsyncMock(
    ...
    async def test_empty_ids_returns_empty_list(self) -> None:
        ...
        # When
        result = await load_reservoir_registries_by_ids(mock_processor, [])

        # Then
        assert result == []
        mock_processor.search_reservoir_registries.wait_for_complete.assert_not_called()
    ...
    async def test_returns_registries_in_request_order(self) -> None:
        ...
        # When
        result = await load_reservoir_registries_by_ids(mock_processor, [id1, id2, id3])
    ...
    async def test_returns_none_for_missing_ids(self) -> None:
        ...
        # When
        result = await load_reservoir_registries_by_ids(mock_processor, [existing_id, missing_id])
